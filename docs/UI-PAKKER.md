...
| **Lisens** | MIT |
| **Hentet inn** | `DitherStackedChart` · `DitherGrowthChart` · `RevenueLineChart` · `DitherDonutChart` + `useCanvasSetup`. **08.09 korreksjon:** `DitherGrowthChart` compact som mini linje/growth-boble (forrige → denne måned) på forhandler-toppkort (F3-05). ⛔ `DitherDonutChart` på hjem. **08.09:** donut som månedsboble (trukket tilbake). **07.09:** `DitherGrowthChart` compact var 7d-spark. |
