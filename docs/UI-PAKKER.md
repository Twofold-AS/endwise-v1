...
> (`fixed inset-x-0 bottom-0`, flush mot bunnkant; safe-area er padding
> *inne* i prompt-baren). Idle-stripe er Verksted-kort-bredde med
> **samme hårlinje `#e0e0e0` + 18px**. Stripe = avatar + tekst — ingen
> dropdown/chevron ved kopien. Peek (etter send): AI-boble under stripen,
> strek-ikon **under boblen**, prompt-bar flush nederst med eget Grainient,
> siden synlig — ingen parchment-overlay, ingen scroll-lås. Full-dekning
> kun når brukeren trykker streken (samme ikon **over prompt** lukker til
> peek). Composer-bar er innholdskolonne-bredde, ikke fullskjerm-bakgrunn.
