...
| `ByttPassordSkjema` / `ToFaktorRad` (`apps/web/app/(app)/_shell/`) | **Ingen ny pakke.** Passordbytte (F1-17) og 2FA-status (F1-20) er komposisjon av `PassordFelt` + `StatefulButton` + token-rad. De bor i `_shell` fordi `_shell/profil-kort.tsx` allerede deles mellom Settings › Profil og mekanikerens «Meg» — to kopier av sikkerhetsfeltene ville fått hver sin validering. Reglene (`validerByttPassord`, `toFaktorStatusTekst`, `etter2faBekreftet`) ligger i `@endwise/auth` som rene funksjoner, samme grep som `password-reset.ts` |
...
trykker «Ny tilfeldig» i Settings › Profil (48px, `bevegelse="hover"`). Ikke på widget/kunde.
