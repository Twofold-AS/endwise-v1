...
| Forhandler pulse-hjem (`_shell/phone-home-dealer.tsx`, `_shell/pulse-kort.tsx`, `_shell/phone-home-pulse.ts`, F3-05 08.09.2026) | **Ingen ny pakke.** **08.09 kveld (CODE-GO):** to-delt toppkort (`PulseHeroFlate`). Del 1: ukedag + grå dato + Planlagt/Pågår/Ferdig (live). Del 2: `PulseEndringerLenke` → `/timeplan/endringer` med ekte `[AVVIK `-telling (0 vises) + Amicro `DitherDonutChart` 08–20 Oslo (`#141414`/`#e0e0e0`). Godkjenn på Endringer er økt-lokal stub (ingen notes-update). Analyser nederst. Ikonplater `#ffffff`. ⛔ mock-badge. ⛔ `#0066ff` på sirkelen. ⛔ Avvik+Forespørsel-par på kortet. ⛔ #114/#119. |
