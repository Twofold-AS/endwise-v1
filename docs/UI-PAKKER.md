...
| Auth-feltene (`apps/web/app/_auth/felter.tsx`: `Field`, `INPUT`) + `AuthMerke` | **Ingen ny pakke.** 08.09.2026 Mobbin + polish: felt er `bg-inset` (`#f0f0f0` / mørk `#262626`), `rounded-[16px]`, `min-h-[52px]` padding sm/md, fokus **2px hvit kant** (`border-2` / `focus:border-white` + synlig `#ffffff`-ring; ink-ytrekant på lyst lerret). `/signin` er uten ytterkort — innhold på lerret, horisontalt sentrert, litt over midten. `AuthMerke` er mask + `bg-fg` (synlig i mørkt). `packages/ui` sin `Input` er `h-10`/`rounded-md` — feil mål. |
