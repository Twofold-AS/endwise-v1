...
| Sidebar-først shellet (`apps/web/app/(app)/_shell/`: `nav.ts`, `sidebar.tsx`, `sidebar-header.tsx`, `side-piller.tsx`, `top-bar.tsx`) + destinasjonene `saker/`, `samarbeid/`, `analyse/`, `endwise/` (inkl. `/endwise` oversikt, `/endwise/flagg`, `/endwise/team`, `/endwise/verksted/[slug]`, F1-07/F0-04/F5-11), `innstillinger/*`, `oppstart/` (F5-26 eier-veiviser) | App-nivå **komposisjoner** (F5-13, Jonas IA 28.08). Bygget av `@endwise/ui`-komponenter: `DropdownMenu`, `Dialog`, `Switch`, `Badge`, `BevelButton` + ikon-barrel. **Ingen visningsvelger.** Piller (`SidePiller`) er samme `role="tablist"` / `rounded-pill` som Innstillinger — shadcn `Tabs` er underline New York og ville restyles uansett. Landing åpner første pille. Hjelp er `/support`, ikke slideren. Ingen ny Admin-tab. Tokens uendret. `/endwise` og `/endwise/flagg` er samme mønster: `CardShell` + `Badge` + `Switch`/`StatefulButton`. `/endwise/team` bruker `Field` + native radio + `StatefulButton` + `Badge` «Hoved-admin». `/oppstart` er visningsnavn · avatar · team på `CardShell`. |
