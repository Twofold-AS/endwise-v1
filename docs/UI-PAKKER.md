...
| Pakke | Rolle |
|---|---|
| `@endwise/widget-tokens` | `--ew-*`-tokens (lys/mørk/aksent). ✅ **Lyst tema er standard** (eier 03.08.2026). **06.09.2026:** Attio-farger — ink `#1c1d1f` (tekst + CTA-fyll), Action Blue `#407ff2` (lenker/aktiv), Focus Blue `#94b9ff` (ring), ash `#f3f4f6`, stone/slate `#e4e7ec`/`#d3d8df`, Overcast `#8f99a8`. Logogrønnen `#1ED27D` er merkevare i logo.svg. Nye tokens: `focus`, `ink-utility`, `surface-2`, `border-strong`, `fg-faint`, `accent-dim`, `warn/danger/success`, `glass-*`, `radius-xl/pill` |
...
| **Tekst** | ink `#1c1d1f` · Overcast `#8f99a8` | `text-fg` · `text-fg-muted` |
| **Lyst (ONLY)** | ash-panel `#f3f4f6` · sidebar/kort `#ffffff` · valgt `#ededed` | `bg-bg` · `bg-sidebar` · `bg-sidebar-active` |
...
bruker `logo.svg` uten markeds-masken. `--primary` / `--ew-ink-utility` er **ink `#1c1d1f`** (CTA-fyll).
`--ew-accent` er **Action Blue `#407ff2`** (lenker/aktiv). `--ew-focus` / `--ring` er **Focus Blue `#94b9ff`**.
Ikke grønn, ikke roadmap-rød `#EE2924`. ⛔ Ronny-kropp.

`--ew-success` (`#15B042` / `#1ED27D` i mørkt) er informasjon, ikke knappfarge.
`--ew-accent-soft` = **`#eaf1ff`** i lyst tema (aktiv-wash). Sidebar-wash `#f8f8f8`.
