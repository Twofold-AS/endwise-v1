...
| Forhandler pulse-hjem (`_shell/phone-home-dealer.tsx`, `_shell/pulse-kort.tsx`, `_shell/phone-home-pulse.ts`, F3-05 08.09.2026) | **Ingen ny pakke.** Fem flater: toppkort + to radkort + ansatte-rad + Jobb-flis. Flate = eksisterende Mobbin-tokens (`PHONE_HERO_FYLL` / `PHONE_DEST_FYLL`). Boble = Amicro `DitherGrowthChart` compact (forrige → denne; allerede i §2). Ikonplater = fast `#ffffff` (ikke tema-`bg-card`). + Jobb = én linje, samme plate. Innboks kun meldingstall. Ikoner 22px. Mangel på historikk: shadcn `Badge` `variant="secondary"`-pille `mock`. ⛔ donut på hjem. ⛔ mini-stats/trend. ⛔ «For lite data». ⛔ Svarhastighet / Timeplan-gulv / Team-liste / Org-Hjelp-footer. shadcn Card er New York-boxed. ⛔ dither-kit. ⛔ pip. Chrome og Ronny/profil urørt. |
