...
> ### ⚠️ Galaxy er Oppgrader + Ronny-stripe + full-åpen; Grainient er ute av Ronny (03.09.2026)
> Galaxy (`react-bits` Galaxy-JS-CSS + `ogl`) er animert bakgrunn på
> workshop-stripen (telefon ~44px, desktop `h-control` / 32px), peek/logg-rammen
> under stripen, **og** full-åpen (`data-ronny-flate`) — samme komponent som
> Oppgrader, `density={2.5}` på `#111` (Oppgrader er `density={1}`).
> Grainient er **ikke** på stripe, peek eller composer. Idle/peek-composer nederst
> (`fixed inset-x-0 bottom-0`, full viewport-bredde, `rounded-none` mot
> skjermkant; safe-area er padding *inne* i baren) er **transparent** —
> flyter over parchment, uten Grainient eller Galaxy. Idle-stripe er
> Verksted-kort-bredde med **samme hårlinje `#e0e0e0` + 18px**. Stripe =
> avatar + tekst — ingen dropdown/chevron. Peek: AI-boble under stripen,
> strek **under boblen**, prompt-bar full-bleed og transparent, siden
> synlig. Full: **Galaxy** på mørk `#111` — prompt uten eget WebGL. ⛔ Ikke Galaxy på
> dealer-pergament. Samme `Galaxy`-komponent som Oppgrader — ikke en kopi.
> Samme strek over prompt lukker til peek. Idle-tekst: «Trykk på KI-Ronny».
...
| **Bakgrunn (AI-stripe + full)** | Galaxy (`ogl`) | KI-Ronny-stripe og peek-panel: 44px telefon / 32px desktop, samme `Galaxy` som full-åpen (`density={2.5}`, `#111`). Ronny+tekst midtstilt. Grainient ute. ShaderGradient er ute. Ikke dealer-hero. |
...
| **Oppgrader-CTA + Ronny** | Galaxy (`ogl`) | React Bits Galaxy: oval Oppgrader-knapp (`density={1}`, `#111`) og Ronny-stripe/full (`density={2.5}`, `#111`). Én komponent. Ikke Grainient. Ikke dealer-pergament. |
...
| Sidebar-først shellet (`apps/web/app/(app)/_shell/`: `nav.ts`, `sidebar.tsx`, `sidebar-header.tsx`, `phone-shell.tsx`, `phone-home.ts`, `phone-home-dealer.tsx`, `phone-home-mekaniker.tsx`, `phone-kort.tsx`, `phone-chrome.ts`, `phone-h-scroll.tsx`, `seksjon-bar.tsx`, `seksjon-sti.ts`, `side-piller.tsx`, `oppgrader-pille.tsx`, `tilbake-pil.tsx`) + destinasjonene `saker/`, `samarbeid/`, `analyse/`, `endwise/` (inkl. `/endwise` oversikt, `/endwise/flagg`, `/endwise/team`, `/endwise/verksted/[slug]`, F1-07/F0-04/F5-11), `innstillinger/*`, `oppstart/` (F5-26 eier-veiviser) | App-nivå **komposisjoner** (F5-13). Bygget av `@endwise/ui`. **01.09.2026 kveld (Mikael IA+auth):** Hjelp-`TipCard` er ute. Nederst sitter Grainient-`OppgraderPille` («Oppgrader til {neste}» / «Enterprise»). Forhandlernavn er ren `<h1>`, ikke kort. Telefon: sidebar-ikon ved logoen (24px åpen/lukket), Tilbake er ordet (`history.back()`), logo 24px = Hjem. `DestinasjonSeksjonBar` under Ronny på alle destinasjoner. Samarbeid borte fra telefon-hjem. Piller er Organisasjon-stil (`bg-sidebar-active`, wrap, ingen slider). Innboks: én verktøylinje i lista, telefon skjuler lista når en tråd er åpen. Åpen tråd: dest-bar = Tilbake + slett + Inviter ansatt. Hero på Verkstedet = forhandlernavn. **01.09.2026 kveld:** overlay-sidebar begge breakpoints, Grainient Ronny-stripe, lys-only. `/bot` lever som URL, ikke i nav. |
| `TilbakePil` (`apps/web/app/(app)/_shell/tilbake-pil.tsx`) | **Ingen ny pakke.** Mikael 02.09.2026: ordet «Tilbake» uten pil/lucide/chevron. Logo og sidebar-toggle er 24px — tyngre enn teksten. |
...
| `RonnyHandtak` (`_workshop/ronny-ikoner.tsx`) | **Ingen ny pakke.** Horisontal strek (iOS-grabber). Peek: under AI-boblen. Full: like over prompt-boksen. Samme kontroll åpner/lukker. shadcn Sheet-håndtak er ikke hentet. |
| `.ronny-tenker-tekst` (stripe-label) | **Ingen ny pakke.** shadcn AI Elements Conversation/Shimmer er ikke hentet (§7). «Ronny tenker…» erstatter idle-kopien («Trykk på KI-Ronny») ved avataren, i takt med BloubBot `state='thinking'`. Ingen egen tenke-rad i peek/logg. |
...
| `OppgraderPille` (`apps/web/app/(app)/_shell/oppgrader-pille.tsx`) | **Galaxy** (§1). Oval knapp, Galaxy klippet inne, svart `#111`. Tekst fra `nesteTier`/`oppgraderKnappetekst` i `@endwise/modules/billing` — ingen priser. Lenke til Organisasjon › Abonnement. Grainient er ute av denne knappen. |
...
| Workshop-stripe (`_workshop/workshop-bloub.tsx` + `galaxy.tsx`, 01.09.2026 kveld) | **Galaxy** på stripe, peek-panel og full-åpen (`density={2.5}` på `#111`, samme komponent som Oppgrader — `RonnyGalaxy` i fila, ikke en kopi). Grainient ute av Ronny. Telefon ~44px, desktop 32px. Hvit KI-Ronny midtstilt. **03.09.2026 (Mikael IA Galaxy-stripe):** Stripe/peek-panelet byttet fra Grainient til samme Galaxy som full-åpen. Composer forblir transparent. **03.09.2026 (Mikael IA Galaxy-flate):** Full-åpen `data-ronny-flate` er Galaxy, ikke Grainient. Peek-prompt fortsatt transparent. ⛔ Galaxy på dealer-sider. **03.09.2026 (Mikael IA flytende prompt):** Peek/dock-composer (`data-ronny-composer` / `data-ronny-prompt-flate`) er transparent — ingen eget Grainient, parchment synlig gjennom. **02.09.2026 (Mikael IA full-bleed/ett grainient):** Prompt-bar full viewport-bredde, `rounded-none` mot bunnkant. Peek-composer uten eget Grainient. Full har kun flatens Grainient — ingen andre canvas. **02.09.2026 (Mikael IA strek/flush):** Stripe uten dropdown-ikon. Peek: strek under AI-boble. Full: strek like over prompt. Prompt-bar flush mot bunn (safe-area inne i grainient-flaten). **02.09.2026 (Mikael IA peek-tilbake):** Peek uten overlay — svar øverst, composer-kort nederst, siden synlig. Full kun via strek. **02.09.2026 (Mikael IA overlay/blur):** Fast ugjennomsiktig overlay under chrome; side-scroll låst. Composer full bredde. Gradual Blur på overlapping logg. Håndtak over input. «Trykk på KI-Ronny». Sidebar lukker til idle. **02.09.2026 (Mikael IA stripe/full):** Stripe og idle/peek-panel har samme hårlinje + 18px som prompt-kortet. Full-åpen = viewport-grainient under topbar (ikke `VERKSTED_INNHOLD`). Tenking kun i stripe-tekst. Composer løftet (`COMPOSER_BUNN`). Mer luft boble↔håndtak. **02.09.2026 (Mikael IA bredde):** Idle/peek-grainient er `VERKSTED_INNHOLD` — samme kolonne som Verksted-kortet (`max-w-[520px] px-3` / `md:max-w-[1120px] md:px-8`). Prompt lavere (`min-h-6` / `py-1.5`, felt fortsatt 16px). **02.09.2026 (Mikael IA 7):** 18px radius idle og peek, topp og bunn; samme på prompt-kort. Ingen hvit canvas bak meldinger. Ingen hårlinje under «Spør Ronny». Åpne/lukke: `cubic-bezier(0.32, 0.72, 0, 1)`, ingen bounce. Full-åpen composer uten Grainient. **02.09.2026 (Mikael sticky + iOS):** Stripe-tap åpner sticky Grainient-composer nederst (`data-ronny-composer`, overlay). Send: peek under stripen med bare Ronnys svar. `RonnyHandtak` (strek, tap/dra) åpner full logg. Telefon-logg `text-[14px]` + `gap-6`. Felt ≥16px på telefon (Prompt Input + global CSS). **02.09.2026 (Mikael Ronny-dock):** idle 5s uten `thinking` — tenking kun `submitted`/`streaming`. Full logg er `fixed` overlay under stripen. **02.09.2026 (Mikael t648):** kompakt Prompt Input (1–2 linjer, submit på rad). Submit `#0066cc`. ⛔ Model-picker, globe, vedlegg, tool-liste, Morph, lucide-piler. |
