...
> Mangel på historikk = `mock`-badge, aldri «For lite data».
> Svarhastighet / Timeplan-gulv / gammel Team-flate er foldet inn.
> Chrome + Ronny/profil urørt. Ingen ny pakke. ⛔ pip. ⛔ #114/#119.
...
> Hjem-innhold = pulse v2 (I dag · Innboks · Lager · Ansatte på jobb ·
> +Jobb) i Mobbin-kort (`rounded.md` 24, tint-fill hero uten kant,
> hvilekort hairline-soft). Footer-tekst Organisasjon · Hjelp.
...
| Forhandler pulse-hjem (`_shell/phone-home-dealer.tsx`, `_shell/pulse-kort.tsx`, `_shell/phone-home-pulse.ts`, F3-05 08.09.2026 v2) | **Ingen ny pakke.** I dag + fire linjekort + footer. Flate = Mobbin-tokens (`PHONE_HERO_FYLL` / `PHONE_DEST_FYLL`). Månedsboble = Amicro growth-dither på **låst 72px-canvas** i `PulseMaanedDither` (samme tiled-under-kurve som `DitherGrowthChart`; chart-komponenten maler ikke i 72px-boblen — bitmap blir 300×150 / 0 piksler). Alltid hvit sirkel + ink. Linje = hvit ikonflate + tekst + pil-hale + teller; Innboks har mini-trend (grønn/rød kun der). Mangel på historikk: shadcn `Badge` `variant="secondary"`-pille `mock`. ⛔ «For lite data». ⛔ dither-kit. ⛔ pip. ⛔ Ronny/profil. Chrome urørt. |
