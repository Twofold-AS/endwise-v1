...
| Workshop-stripe (`_workshop/workshop-bloub.tsx` + `grainient.tsx`, 01.09.2026 kveld) | **Grainient** (§1). Telefon ~44px, desktop 32px. Hvit KI-Ronny midtstilt. **02.09.2026 (Mikael t648):** kompakt Prompt Input (1–2 linjer, pille, submit på rad). Første send folder hele boksen til fast samtalehøyde; flere meldinger scroller over prompt (`no-scrollbar`). Sirkel = fullscreen. **02.09.2026 (Mikael t646+t647):** én Grainient-boks `overflow-hidden rounded-[18px]`; indre hvitt panel også 18px. Overlay over dest-bar. `RonnyPil` fra `chevron-down.svg`. Idle 2.5s. Submit `#0066cc`. Escape: utvidet → dock → stripe. ⛔ Model-picker, globe, vedlegg, bunndock, lucide-piler. |
