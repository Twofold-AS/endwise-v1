...
| Claude-innbygging innhold (`apps/web/app/(app)/_innbygging/`, 15.09.2026 Mikael OVERRIDE) | **Ingen ny pakke.** Komposisjon av shadcn `Dialog`/`Switch`/`Button`, `CardShell`, `InnstillingSeksjon`, `StatusMerke` (myk tint, ingen pip). Delt `innholdPilleKlasse` (ink valgt) for innboks/Endringer/Ny jobb — ikke dest-underline, ikke Claude-chrome. Tall #7 = 2×2 surface-2-plater. Egen kode fordi ingen pakke dekker Endwise-katalogene (status, endringstyper, Mobbin 24). Chrome urørt. |
