...
| Sidebar-først shellet (`apps/web/app/(app)/_shell/`: `nav.ts`, `sidebar.tsx`, `sidebar-header.tsx`, `phone-nav.tsx`, `phone-chrome.ts`, `seksjon-bar.tsx`, `seksjon-sti.ts`, `side-piller.tsx`, `top-bar.tsx`) + destinasjonene `saker/`, `samarbeid/`, `analyse/`, `endwise/` (inkl. `/endwise` oversikt, `/endwise/flagg`, `/endwise/team`, `/endwise/verksted/[slug]`, F1-07/F0-04/F5-11), `innstillinger/*`, `oppstart/` (F5-26 eier-veiviser) | App-nivå **komposisjoner** (F5-13, Jonas IA 28.08, Mikael telefon 28.08, Mikael Innboks 28.08 kveld). Bygget av `@endwise/ui`-komponenter: `DropdownMenu`, `Dialog`, `Switch`, `Badge`, `BevelButton` + ikon-barrel. **Ingen visningsvelger.** Piller (`SidePiller`) er samme `role="tablist"` / `rounded-pill` som Innstillinger — shadcn `Tabs` er underline New York og ville restyles uansett. Landing åpner første pille. Hjelp er `/support`, ikke slideren. Telefon-hovedmeny er `h-row` + pinnest `logo.svg` + `touch-pan-x` + målt end-spacer; ingen Handlinger. Top-bar 2 er `h-control` på desktop (Organisasjon + Innboks › Oversikt); på telefon `py-1.5` med piller som har `py-1` (ikke flush). Ingen ny Admin-tab. Tokens uendret. `/endwise` og `/endwise/flagg` er samme mønster: `CardShell` + `Badge` + `Switch`/`StatefulButton`. `/endwise/team` bruker `Field` + native radio + `StatefulButton` + `Badge` «Hoved-admin». `/oppstart` er visningsnavn · avatar · team på `CardShell`. |
| Meldings- og AI-flatene (`apps/web/app/(app)/`: `innboks/page.tsx` + `innboks/[id]/page.tsx` + `_lib.ts` + `_chrome.tsx` / `_modus.tsx` + `_ny-samtale.tsx` + `_inbox-sidebar.tsx`, `endwise/innboks/`, `integrasjoner/ai/page.tsx`, `_lib/use-event-stream.ts`) | App-nivå **komposisjoner**, ikke primitiver: shadcn/beUI-knapper (`Button`, `StatefulButton`) + matrix-loaders + `@endwise/ui`-ikoner + `CardShell`/`CardMedia`. F5-11 (23.08.2026): `/endwise/innboks` gjenbruker innboks-chrome (`modus=endwise`). F5-14 (25.08.2026, Jonas + Mikael): Ny samtale er e-post-aktig liste + tre piller (Kunde · Intern · Support). Intern = verkstedsgulvet (`mechanics.list`), ingen fjerde Mekaniker-pille. <b>28.08.2026 kveld:</b> Innboks bruker samme top-bar 2 som Organisasjon (Oversikt). Part-filtre er ikon-only (`Inbox` / `Users` / `Wrench` / `LifeBuoy`) med `aria-label` Kunder · Intern · Endwise · Alle chatter — shadcn `toggle-group` er ikke hentet inn; aktiv = `bg-sidebar-active` som Organisasjon-piller. Ny samtale er tekst-`Button` i samme rad. `SupportKort` og `EndwiseForhandlerDetaljer` er lokale rader — shadcn har ingen innboksrad med forhandlernavn + muted utdrag + aksentprikk. «Se verkstedet» er en `Link` til `/endwise/verksted/[slug]` (ikke setActive). `use-event-stream.ts` er en **datahook**, ikke UI. Ingen ny pakke tatt inn |
