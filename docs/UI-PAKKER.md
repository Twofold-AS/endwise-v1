...
> peek/logg-rammen under stripen, **og** den sticky composer-en nederst
> (`fixed inset-x-0 bottom-0`, overlay). Lukket stripe har **radius 0**.
> Peek møter stripen firkantet oppe (`rounded-t-none`); ev. rund kun nedre
> hjørner. Expand er horisontal strek (`RonnyHandtak`), ikke pil/sirkel.
> Composer er eget Grainient-felt nederst — ikke inne i peek. Peek viser
> bare Ronnys svar. Full logg = Message/MessageBubble. Telefon-logg 14px;
> alle felt ≥16px (iOS-zoom). Innholdet i panelet er canvas/parchment.
...
| Workshop-stripe (`_workshop/workshop-bloub.tsx` + `grainient.tsx`, 01.09.2026 kveld) | **Grainient** (§1). Telefon ~44px, desktop 32px. Hvit KI-Ronny midtstilt. **02.09.2026 (Mikael sticky + iOS):** Stripe-tap åpner sticky Grainient-composer nederst (`data-ronny-composer`, overlay). Send: peek under stripen med bare Ronnys svar. `RonnyHandtak` (strek, tap/dra) åpner full logg. Telefon-logg `text-[14px]` + `gap-6`. Felt ≥16px på telefon (Prompt Input + global CSS). **02.09.2026 (Mikael Ronny-dock):** idle 5s uten `thinking` — tenking kun `submitted`/`streaming`. Full logg er `fixed` overlay under stripen. **02.09.2026 (Mikael IA A):** lukket = radius 0. **02.09.2026 (Mikael t648):** kompakt Prompt Input (1–2 linjer, submit på rad). Submit `#0066cc`. ⛔ Model-picker, globe, vedlegg, tool-liste, Morph, lucide-piler. |
