...
| Forhandler pulse-hjem (`_shell/phone-home-dealer.tsx`, `_shell/pulse-kort.tsx`, `_shell/phone-home-pulse.ts`, F3-05 09.09.2026) | **Ingen ny pakke.** **09.09 (CODE-GO):** Amicro-halvsirkel (`sweep={π}`) på samme linje som Planlagt/Pågår/Ferdig, etiketter 08…19. Avvik\|Forespørsler = visningsboks (rød/gul, ikke `.ew-modus-plate`). Hvit Endringer + `ChevronRight` → `/jobber?fane=endringer`. **08.09 natt (CODE-GO polish):** sirkel over Endringer, 08–19, `startAngle={π}` klokkevis venstre→høyre. Avvik + Forespørsler nederst (`/avvik`, `/hjelp?fane=forespor`). Jobb: etikett venstre, +ikon høyre. **08.09 kveld (CODE-GO):** to-delt toppkort (`PulseHeroFlate`). Del 1: ukedag + grå dato + Planlagt/Pågår/Ferdig (live). Del 2: `PulseEndringerLenke` → `/timeplan/endringer` med ekte `[AVVIK `-telling (0 vises) + Amicro `DitherDonutChart` (`#141414`/`#e0e0e0`). Godkjenn på Endringer er økt-lokal stub (ingen notes-update). Analyser nederst. Ikonplater `#ffffff`. ⛔ mock-badge. ⛔ `#0066ff` på sirkelen. ⛔ #114/#119. |
