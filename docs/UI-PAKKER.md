...
> (`fixed inset-x-0 bottom-0`, overlay, løftet med safe-area + 16px).
> Idle-stripe er Verksted-kort-bredde med **samme hårlinje `#e0e0e0` + 18px**.
> Peek (etter send): svar under stripen, composer-kort alene nederst
> (eget Grainient i kortet), siden synlig — ingen parchment-overlay,
> ingen scroll-lås. Full-dekning kun når brukeren trykker streken
> (samme ikon over input lukker til peek). Composer-kort er
> innholdskolonne-bredde, ikke fullskjerm-bakgrunn. Håndtak over input.
> Idle-tekst: «Trykk på KI-Ronny». Tenking = stripe-tekst, ikke logg-rad.
...
| `RonnyPil` (`_workshop/ronny-ikoner.tsx`) | **Ingen ny pakke.** Inliner path fra `packages/ui/src/assets/icons/chevron-down.svg` (samme som `icons-v2/arrow-dropdown.svg`), som `NyMeldingIkon`. Ikke lucide, ikke hjemmelaget chevron. |
| `RonnyHandtak` (`_workshop/ronny-ikoner.tsx`) | **Ingen ny pakke.** Horisontal strek (iOS-grabber) for peek/full logg. shadcn Sheet-håndtak er ikke hentet; ingen pakke i kartet dekker «dra-strek på Grainient». |
