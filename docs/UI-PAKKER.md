...
| Sidebar-først shellet (`apps/web/app/(app)/_shell/`: `nav.ts`, `sidebar.tsx`, `top-bar.tsx`, `context-switcher.tsx`, `command-palette.tsx`) + destinasjonene `saker/`, `samarbeid/`, `analyse/`, `endwise/` (inkl. `/endwise/flagg`, F0-04), `innstillinger/*` | App-nivå **komposisjoner** (F5-13). Bygget av `@endwise/ui`-komponenter: `DropdownMenu`, `Dialog`, `Switch`, `Badge`, `BevelButton` + ikon-barrel. **⌘K-paletten er egenskrevet** fordi shadcns `command` krever `cmdk` — ny pakke = §2-endring, ikke godkjent. Paletten er ~60 linjer filtrering over `nav.ts`; `cmdk` kan erstatte den senere uten at kallstedene endres. `/endwise/flagg` er samme mønster: `Switch` + `Badge` + `StatefulButton` + `CardShell` — ingen ny primitiv, native `<select>` som på `/endwise/forhandlere` |
