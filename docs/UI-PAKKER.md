...
> Idle/peek-grainient er **Verksted-kort-bredde** (`VERKSTED_INNHOLD`)
> med **samme hårlinje `#e0e0e0` + 18px** som prompt-kortet.
> Full-åpen er **viewport-grainient under chrome** (ikke smalt kort,
> ett fyll — composer uten eget Grainient). Expand er horisontal strek
> (`RonnyHandtak`), ikke pil/sirkel. Peek viser bare Ronnys svar.
> Tenking = stripe-tekst ved avatar (`Ronny tenker…` + shimmer),
> ikke egen rad i loggen. Full logg = Message/MessageBubble på Grainient
> (ingen hvit canvas bak boblene). Telefon-logg 14px;
> alle felt ≥16px (iOS-zoom). Ikke forhandler-hero, ikke Oppgrader.
...
| `.ronny-tenker-tekst` (stripe-label) | **Ingen ny pakke.** shadcn AI Elements Conversation/Shimmer er ikke hentet (§7). «Ronny tenker…» erstatter idle-kopien ved avataren i stripen (hvit shimmer), i takt med BloubBot `state='thinking'`. Ingen egen tenke-rad i peek/logg. |
