...
> (`fixed inset-x-0 bottom-0`, overlay). Åpen grainient (topp og bunn) er
> **Verksted-kort-bredde** (`VERKSTED_INNHOLD`), ikke full-bleed.
> Idle **og** åpen: **18px radius
> topp og bunn** (ikke radius 0, ikke `rounded-t-none`). Samme 18px på
> prompt-kortet. Expand er horisontal strek (`RonnyHandtak`), ikke pil/sirkel.
> Composer er eget Grainient-felt nederst i idle/peek — ikke inne i peek.
> Full-åpen composer sitter på panelet **uten** eget Grainient (ingen
> dobbel-lag). Peek viser bare Ronnys svar. Full logg = Message/MessageBubble
> på Grainient (ingen hvit canvas bak boblene). Telefon-logg 14px;
...
| `RonnyTenkerTekst` + `.ronny-tenker-tekst` | **Ingen ny pakke.** shadcn AI Elements Conversation/Shimmer er ikke hentet (§7). Samme shimmer-mønster (gradient + `bg-clip-text`) på teksten «Ronny tenker…», i takt med eksisterende BloubBot `state='thinking'`. Ikke en ekstra spinner. |
...
| Workshop-stripe (`_workshop/workshop-bloub.tsx` + `grainient.tsx`, 01.09.2026 kveld) | **Grainient** (§1). Telefon ~44px, desktop 32px. Hvit KI-Ronny midtstilt. **02.09.2026 (Mikael IA bredde):** Åpen grainient (stripe + composer) er `VERKSTED_INNHOLD` — samme kolonne som Verksted-kortet (`max-w-[520px] px-3` / `md:max-w-[1120px] md:px-8`), ikke full-bleed. Prompt lavere (`min-h-6` / `py-1.5`, felt fortsatt 16px). Tettere gap boble↔håndtak. **02.09.2026 (Mikael IA 7):** 18px radius idle og åpen, topp og bunn; samme på prompt-kort. Ingen hvit canvas bak meldinger. Ingen hårlinje under «Spør Ronny». «Ronny tenker…» med shadcn-shimmer + BloubBot `thinking`. Åpne/lukke: `cubic-bezier(0.32, 0.72, 0, 1)`, ingen bounce. Full-åpen composer uten Grainient. **02.09.2026 (Mikael sticky + iOS):** Stripe-tap åpner sticky Grainient-composer nederst (`data-ronny-composer`, overlay). Send: peek under stripen med bare Ronnys svar. `RonnyHandtak` (strek, tap/dra) åpner full logg. Telefon-logg `text-[14px]` + `gap-6`. Felt ≥16px på telefon (Prompt Input + global CSS). **02.09.2026 (Mikael Ronny-dock):** idle 5s uten `thinking` — tenking kun `submitted`/`streaming`. Full logg er `fixed` overlay under stripen. **02.09.2026 (Mikael t648):** kompakt Prompt Input (1–2 linjer, submit på rad). Submit `#0066cc`. ⛔ Model-picker, globe, vedlegg, tool-liste, Morph, lucide-piler. |
