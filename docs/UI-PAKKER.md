...
> ### ⚠️ Grainient-gotcha (29.08.2026)
> `apps/web/app/globals.css` MÅ ha `@import "@endwise/ui/grainient.css";`. Grainient er react-bits
> WebGL (`ogl`) — komponenten setter `.grainient-container` og shader-uniforms. Uten CSS-importen
> fyller ikke bakgrunnen kortet (100% × 100% av hero-kortet, radius 12 — ikke 1080-kvadrat).
> `ogl` må ligge i **både** `packages/ui` og `apps/web` (Next transpilerer UI-kilde i appens
> resolusjonskontekst, samme felle som `motion`/`recharts`). Lys og mørk bruker samme
...
| **Maskot-lab** | Morph Bot-runtime + Endwise-blob | Intern `/bot`. Ikke produkt-avatar. Se §11 |
...
| Sidebar-først shellet (`apps/web/app/(app)/_shell/`: `nav.ts`, `sidebar.tsx`, `sidebar-header.tsx`, `phone-shell.tsx`, `phone-home.ts`, `phone-home-dealer.tsx`, `phone-home-mekaniker.tsx`, `phone-kort.tsx`, `phone-chrome.ts`, `phone-h-scroll.tsx`, `seksjon-bar.tsx`, `seksjon-sti.ts`, `side-piller.tsx`, `top-bar.tsx`) + destinasjonene `saker/`, `samarbeid/`, `analyse/`, `endwise/` (inkl. `/endwise` oversikt, `/endwise/flagg`, `/endwise/team`, `/endwise/verksted/[slug]`, F1-07/F0-04/F5-11), `innstillinger/*`, `oppstart/` (F5-26 eier-veiviser) | App-nivå **komposisjoner** (F5-13, Jonas FILL 29.08, Jonas IA 28.08, Mikael telefon 28.08, Mikael Innboks 28.08 kveld). Bygget av `@endwise/ui`-komponenter: `DropdownMenu`, `Dialog`, `Switch`, `Badge`, `BevelButton` + ikon-barrel. **Ingen visningsvelger.** Piller (`SidePiller`) er samme `role="tablist"` / `rounded-pill` som Innstillinger — shadcn `Tabs` er underline New York og ville restyles uansett. Landing åpner første pille. Hjelp er `/support`, ikke slideren. <b>29.08.2026:</b> telefon-hovedmeny er kort-hjem (`PhoneShell`): hvit topp + logo, bevel (avatar · navn · logg ut), `safe-area` + `dvh`/`svh`. Ingen horisontal scroller, hamburger, Mer-sheet eller bunnbar. Destinasjonskort er fylt `bg-accent` (radius 12, `text-title` + `text-[12px]` meta). Innboks-filter beholder `PhoneHScroll`. Organisasjon-piller wrapper på telefon. Desktop-sidebar urørt. Tokens uendret. `/endwise` og `/endwise/flagg` er samme mønster: `CardShell` + `Badge` + `Switch`/`StatefulButton`. `/endwise/team` bruker `Field` + native radio + `StatefulButton` + `Badge` «Hoved-admin». `/oppstart` er visningsnavn · avatar · team på `CardShell`. |
| Meldings- og AI-flatene (`apps/web/app/(app)/`: `innboks/page.tsx` + `innboks/[id]/page.tsx` + `_lib.ts` + `_chrome.tsx` / `_modus.tsx` + `_ny-samtale.tsx` + `_inbox-sidebar.tsx`, `endwise/innboks/`, `integrasjoner/ai/page.tsx`, `_lib/use-event-stream.ts`) | App-nivå **komposisjoner**, ikke primitiver: shadcn/beUI-knapper (`Button`, `StatefulButton`) + matrix-loaders + `@endwise/ui`-ikoner + `CardShell`/`CardMedia`. F5-11 (23.08.2026): `/endwise/innboks` gjenbruker innboks-chrome (`modus=endwise`). F5-14 (25.08.2026, Jonas + Mikael): Ny chat er e-post-aktig liste + tre piller (Kunde · Intern · Support). Intern = verkstedsgulvet (`mechanics.list`), ingen fjerde Mekaniker-pille. <b>28.08.2026 kveld (telefon/PC):</b> Ingen Oversikt på Innboks. Telefon top-bar 2 = ikon+tekst-filtre + Ny chat (`MessageSquarePlus`). Desktop: ingen innboks-top-bar 2; ikon-only i list-header; tom detalj = stor `Inbox` + «Ingen valgte meldinger». Filtre er ikke destinasjoner (`InboxFilterProvider`). shadcn `toggle-group` er ikke hentet inn; aktiv = `bg-sidebar-active`. `SupportKort` og `EndwiseForhandlerDetaljer` er lokale rader — shadcn har ingen innboksrad med forhandlernavn + muted utdrag + aksentprikk. «Se verkstedet» er en `Link` til `/endwise/verksted/[slug]` (ikke setActive). `use-event-stream.ts` er en **datahook**, ikke UI. Ingen ny pakke tatt inn |
...
| Telefon kort-hjem + mekaniker-accordion (`_shell/phone-home*.tsx`, `_shell/phone-kort.tsx`, `_shell/phone-shell.tsx`, F5-13 29.08.2026) | **Ingen ny pakke for kortene.** Destinasjonskort er samme token-komposisjon som `CardShell` (`bg-card` / `text-fg` / `border-border` / `rounded-xl`) + ikoner fra `@endwise/ui`. Ikke `bg-accent`/`text-accent-fg`. **29.08 natt:** Min dag-hero og Detaljer-accordion er borte — Dine jobber er egen rute. Bevel er i dokumentflyt (ikke sticky). Tilbake sitter på logo-raden. Forhandler-kortet bruker **Grainient** (§1), ikke egen gradient. **29.08 kveld:** bevel er siste barn med `mt-auto` i en kolonne som minst er synlig høyde (`min-h-dvh` / `flex-1` på main) — nederst når innholdet er kort, etter innholdet når det er langt. Grainient er #777/#333/#111 i begge tema, lys tekst. **29.08 kveld (Mikael mekaniker):** stort Dine jobber-kort med tre faste `h-row-store`-spor + Bevel «Se alle jobber»; Lager full bredde under. Ingen pakke dekker «dagens jobber i faste spor». |
...
| Bot-lab (`apps/web/app/(app)/bot/`, F6-29, 31.08.2026) | **Ingen ny npm-pakke.** Runtime er vendorisert Morph Bot (Web Component). Geometrien er original Endwise-blob (`packages/ui/src/morph-bot/endwise-blob.js`) — ikke x.ai. Siden er intern gjennomgang: seks primærchips + 39 tilstander + 14 Morphs. shadcn har ingen maskot. ⛔ Erstatter ikke `Avatar`/blobatar. Se §11. |
...
## 11. Morph Bot-runtime + Endwise-blob (intern lab, 31.08.2026)

| | |
|---|---|
| **Brukes til** | Intern gjennomgang av maskot-animasjon på `/bot` |
| **Installasjon** | **Vendorisert runtime** (kopiert inn — ikke npm). Oppstrøms har ikke `license`-felt |
| **Kilde (runtime)** | https://github.com/scrya-com/grokbot-animation `component/` |
| **Pin** | commit `031a583194be0e2cb9164b25dceb4674a57f35ec` |
| **Ligger i** | `packages/ui/src/vendor/morph-bot/` (runtime) · `packages/ui/src/morph-bot/endwise-blob.js` (original Endwise-geometri) |
| **Lisens** | Runtime: ⚠️ ingen license-felt oppstrøms, derfor vendor + `private: true`. Endwise-blob er vår |
| **Hentet inn** | Kun runtime: `morph-bot.js`, `grok-bot-engine.js`, `original-data.js`, `catalog.js`, `materials.js`, `runtime/*`, `morph-bot.d.ts`. Ikke lab-editor, Flutter, ANALYSIS eller deres CSS-tema |
| **Status** | Intern lab. ⛔ **Ikke produkt-avatar.** `Avatar` / blobatar er urørt |

Jonas låste **Endwise-blob**: én svart kropp (96-pt), to hvite fylte øyne (48-pt), ingen munn/pupiller/bryn. Seks sett: idle · tenker · lytter · laster · feirer · alarm. Form-id `endwise`. Lyst: kropp `#111111` / øyne `#ffffff`. Mørkt: invertert. Blink er motorens Y-skala, ikke en tredje path.

På `/bot` spleises Endwise-data inn før motoren starter (`endwise-splice.js`): `SHAPES.endwise` + `SHAPES.blob` byttes til body96, `EXPRESSIONS` fylles med de seks øye-settene (25 plasser paddet), `EXPRESSION_POOLS` peker hver av de 39 tilstandene på nærmeste av de seks. Ingen 18-formsvelger. Morph-oneshots beholdes.
