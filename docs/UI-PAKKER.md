...
| Workshop-stripe (`_workshop/workshop-bloub.tsx` + `grainient.tsx`, 01.09.2026 kveld) | **Grainient** (§1). Telefon ~44px, desktop 32px. Hvit KI-Ronny midtstilt. **02.09.2026 (Mikael Ronny-dock):** idle 5s uten `thinking` — tenking kun `submitted`/`streaming`. «Se hele» på grainient under Prompt-kortet (ikke i composer-footer) åpner `utvidet` som `fixed` overlay under stripen, full bredde over sidebar. Prompt-kort = `PHONE_KORT_FYLL` + 18px + `#fff`/`#e0e0e0`/`#1d1d1f`, `min-h-[8.75rem]`, flush `max-w-[520px] px-3` / `md:max-w-[1120px] md:px-8`. Første send vokser tråden i dock. **02.09.2026 (Mikael IA A):** lukket = radius 0. Åpen = Grainient-kort 18px, dashboard-px, Prompt animerer inn. **02.09.2026 (Mikael t648):** kompakt Prompt Input (1–2 linjer, submit på rad). **02.09.2026 (Mikael t646+t647):** `RonnyPil` fra `chevron-down.svg`. Submit `#0066cc`. ⛔ Model-picker, globe, vedlegg, bunndock, lucide-piler. |
