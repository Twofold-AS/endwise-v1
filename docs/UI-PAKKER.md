...
| Telefon kort-hjem + mekaniker-accordion (`_shell/phone-home*.tsx`, `_shell/phone-kort.tsx`, `_shell/phone-shell.tsx`, F5-13 29.08.2026) | **Ingen ny pakke.** Fylte destinasjonskort er token-komposisjon (`bg-accent` / `text-accent-fg` / `rounded-xl`) + ikoner fra `@endwise/ui`. shadcn `Card` er outline/surface og ville restyles til aksent uansett. Mekaniker «Detaljer» er eksklusiv accordion i React-state (én åpen) — shadcn `Accordion` er ikke hentet: den er underline/animation New York og ville likevel restyles til fylte rader uten navigasjon. Book for kunde på Verkstedet-jobbkortet er merkelapp, ikke gjeste-booking. |
