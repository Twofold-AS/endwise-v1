...
> **05.09 kveld (Mikael):** Ronny-avatarer bytter bare ansikt/humør (`state='idle'`).
> ⛔ thinking/alert/notify på bot. PhoneShell er `fixed` z-60; åpen sidebar starter
> under `h-row`, så merke og Ronny|toggle ikke hopper.
...
| `.ronny-tenker-tekst` (stripe-label) | **Ingen ny pakke.** shadcn AI Elements Conversation/Shimmer er ikke hentet (§7). Tittelen forblir «Ronny»; `sr-only` «Ronny tenker…» når chat streamer. Avataren bytter bare uttrykk (`state='idle'`). ⛔ thinking/alert/notify på bot. |
