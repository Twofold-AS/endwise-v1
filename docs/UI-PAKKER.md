...
> ### ⚠️ Grainient er KUN bot-stripen (01.09.2026 kveld)
> Grainient (`react-bits` Grainient-JS-CSS + `ogl` ^1.0.11) er tilbake **bare** som
> animert bakgrunn på workshop-stripen (`h-control` / maks 32px). Ikke forhandler-hero.
> ShaderGradient er ute. Importer `@endwise/ui/grainient.css`. `ogl` i både
> `@endwise/ui` og `@endwise/web`, og i `transpilePackages`. Hvit bloub i stripen
> (`color="#ffffff"`, `paper="#111111"` for øyne), ingen sirkel-chip.
...
| **Bakgrunn (AI-stripe)** | Grainient (`ogl`) | 32px workshop-stripe. ShaderGradient er ute. Ikke dealer-hero. |
...
| Workshop-stripe (`_workshop/workshop-bloub.tsx` + `grainient.tsx`, 01.09.2026 kveld) | **Grainient** (§1). 32px (`h-control`), full bredde, hvit tekst, hvit bloub ytterst til høyre uten sirkel-chip (`color="#ffffff"`, `paper="#111111"`). ShaderGradient ute. Chat er eksisterende workshop (`/chat/workshop`, sidekontekst, ⛔ Quick-skriving). Samme stripe på telefon (under toppbaren) og desktop. |
