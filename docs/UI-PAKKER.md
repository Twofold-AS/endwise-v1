...
| `ProduktRamme` på markedssiden (`apps/web/app/_markeds/`, F5-35, Jonas 05.09.2026) | **Ingen ny pakke, og ingen pakke å hente.** Fast spor til UI-skjermbilde: `aspect-[16/10]` (desktop) / `aspect-[9/19]` (telefon), `rounded-[14px]`, `border-border`. Uten `kilde` = lys UI-plassholder; med statisk import = `next/image` i samme boks (ingen reflow). ⛔ Ikke stock-mekaniker. ⛔ Ikke arkitektur-JPEGene som «produktskudd». Primær CTA er ink `#1c1d1f` (`bg-primary` → `--ew-ink-utility`) — Mikael Attio-farger 06.09.2026. Action Blue `#407ff2` er lenker. Ikke logogrønn. Offentlig merke (`Merke` i `_markeds/markeds-chrome.tsx`) er ink `bg-fg` / `text-fg` via maske på `logo.svg` — Mikael 05.09.2026 etter #133. Dealer-chrome bruker `logo.svg` uten denne masken. |
