...
| Intern testbutikk (`apps/web/app/(app)/butikk/`: `page.tsx`, `kasse/page.tsx`, `_kurv.ts`, F10-03) | App-nivå **komposisjon**, ingen ny pakke. Samme skall som Lager › Deler: `CardShell`, shadcn `Button`, beUI `StatefulButton`, native `input type="number"` med `h-control`/`rounded-control`. Handlekurven er `sessionStorage` — shadcn har ingen handlekurv, og en egen cart-pakke ville vært en §2-sak for en intern preview. Egen kode fordi ingen pakke dekker «lagerkatalog + Stripe test-kasse». Ikonet `ShoppingCart` lagt i lucide-barrelen. |
