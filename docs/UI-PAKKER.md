...
> ⚠️ **AKSENT-ROLLER ER SCOPET (07.09.2026, Mikael Linear dark).**
> **Dealer/app-skall** (`data-theme="dark"` på chrome-roten, ikke `<html>`): Void canvas
> `#08090a` · Carbon nav/kort `#0f1011` · Obsidian løftet `#161718` · Graphite hårlinje
> `#23252a`. Tekst paper/mist/fog. **Acid Lime `#e4f222`** er eneste fylte kromatiske CTA
> og aktiv-nav-indikator; tekst på CTA = void `#08090a`. Støttende aksenter
> (pulse/coral/teal/iris/lavender) kun på tags/serier. Inter 300/400/510/590 (aldri 700+).
> Kort 12 · knapp/input 6 · pille 9999 · badge 4. Hårlinjer, ikke drop-shadow.
> **Offentlig landing** holder Apple `#0066cc` (hardkodet i `_markeds/cta.ts`).
> Lyst `--ew-*` (Attio-ink / Action Blue) lever videre for widget/landing.
> ⛔ Action Blue / ink som app-CTA. ⛔ lime på Apple-landing. ⛔ Attio-layout / Fluid / frost (#138).
> ⛔ Ronny-kropp / logogrønn `#1ED27D`. «Ny»-badgen er RØD (tekstbadge).
> Logogrønnen `#1ED27D` er urørt (bor i logo.svg).
> Ingen tema-toggle. `<html data-theme="light">` står; appen overstyrer på skallet.
...
| **Ikoner (chrome/nav/hjem)** | Disarto Regular (vendorisert) | Fylte evenodd-path-er i `assets/icons/`, F5-20-røret. ⛔ Ikke `disarto-icons-react`. Se §6 og `docs/notater/disarto-ikoner.md` |
...
| **Farger** | Linear støttende aksenter i kallstedet: pulse `#27a644`, teal `#02b8cc`, iris `#6366f1`, lavender `#8b5cf6`. ⛔ Acid Lime på grafer. |
...
| Pakke | Rolle |
|---|---|
| `@endwise/widget-tokens` | `--ew-*` + Linear `--color-*` (void…lavender) på `:root`. **07.09.2026:** `[data-theme=dark]` = dealer/app (Acid Lime CTA, void-tekst, Carbon/Obsidian). Lyst `--ew-*` beholdt for landing/widget (Attio-ink / Action Blue). `<html>` er light; app-skall setter dark. Logogrønnen `#1ED27D` er merkevare i logo.svg. |
| Tailwind CSS 4 | `@theme inline` i `packages/ui/src/theme.css` |
| `radix-ui` | Primitivene shadcn bygger på |
| `lucide-react` | Ikon-wrapper og type (`createLucideIcon` / `LucideIcon`). **Én barrel** — apper importerer via `@endwise/ui` (`icons.ts`), aldri `lucide-react` eller `disarto-icons-react` direkte. ⚠️ **07.09.2026 (Mikael GO):** chrome / sidebar / hjem bruker **Disarto Regular** (MIT, pin `d536bd5`), vendorisert som SVG i `src/assets/icons/` og koblet inn via `scripts/build-icons.ts` → `icons.generated.ts` (F5-20). Disarto er filled-path, ikke lucide-strek — det er med vilje. ⛔ Ikke et andre ikon-API. Mapping: `docs/notater/disarto-ikoner.md`. Fem mangler fortsatt (`hard-hat` · `handshake` · `bike` · `sailboat` · `clock-arrow-up`). Regenerer: `pnpm --filter @endwise/ui build:icons` |
...
| **Titler** | 16px / 20px / 590, tight tracking (app: paper) | `text-title` · `text-heading` |
| **Labels** | 13px / 16px / Regular (400) | `text-label` |
| **Brødtekst** | 17px / 1.47 / Regular (400), mist i app | `text-body` |
| **Knapper (app)** | Acid Lime · void-tekst · 6px · ~10×16 · 14/510. Ghost: graphite + mist | `bg-primary` · `[data-theme=dark]` |
| **Datarad** | 40px | `h-row` |
...
| **Tekst (app)** | paper headings · mist body · fog/ash muted | `text-heading` · `text-fg` · `text-fg-muted` |
| **App-flater** | Void canvas · Carbon nav/kort · Obsidian løftet · Graphite tint | `bg-bg` · `bg-sidebar` · `bg-surface-2` |
| **Landing** | Apple parchment + Action Blue `#0066cc` (hardkodet CTA) | `_markeds/cta.ts` |

**`<html data-theme="light">` står** for landing/widget. Dealer/app-skall setter
`data-theme="dark"` på chrome-roten. Ingen ThemeToggle.
...
merket med ink `#1c1d1f` / `bg-fg` (Mikael 05.09.2026, hex 06.09) — ikke `#1ED27D`. Dealer-chrome
bruker `logo.svg` uten markeds-masken. I **app/dark** er `--primary` / `--ew-ink-utility`
**Acid Lime `#e4f222`** (CTA-fyll) og `--ew-accent-fg` void `#08090a`. I **lyst** (landing/widget)
er CTA fortsatt ink `#1c1d1f` og `--ew-accent` Action Blue `#407ff2`. Offentlig landing-CTA er
hardkodet Apple `#0066cc`. ⛔ lime på landing. ⛔ Action Blue som app-CTA. ⛔ Ronny-kropp.

`--ew-success` (pulse-green i dark) er informasjon, ikke knappfarge.
...
| `ProduktRamme` på markedssiden (`apps/web/app/_markeds/`, F5-35, Jonas 05.09.2026) | **Ingen ny pakke, og ingen pakke å hente.** Fast spor til UI-skjermbilde: `aspect-[16/10]` (desktop) / `aspect-[9/19]` (telefon), `rounded-[14px]`, `border-border`. Uten `kilde` = lys UI-plassholder; med statisk import = `next/image` i samme boks (ingen reflow). ⛔ Ikke stock-mekaniker. ⛔ Ikke arkitektur-JPEGene som «produktskudd». Primær landing-CTA er Apple Action Blue `#0066cc` (hardkodet, Jonas 06.09 — dealer-tokens er Attio-ink). Ikke logogrønn. Offentlig merke (`Merke` i `_markeds/markeds-chrome.tsx`) er ink `bg-fg` / `text-fg` via maske på `logo.svg` — Mikael 05.09.2026 etter #133. Dealer-chrome bruker `logo.svg` uten denne masken. |
...
| Telefon kort-hjem + mekaniker-accordion (`_shell/phone-home*.tsx`, `_shell/phone-kort.tsx`, `_shell/phone-shell.tsx`, F5-13 29.08.2026) | **Ingen ny pakke for kortene.** Destinasjonskort er samme token-komposisjon som `CardShell`. **06.09.2026:** scroll-flate følger `--ew-bg` (ash `#f3f4f6`) — IA/layout urørt. **05.09.2026 (Jonas hard-fasit, dealer-hjem only):** hero-plate radius 16 + stor tittel + I dag/Pågår/Fullført, destinasjonskort radius 14 / hit ≥44 / `touch-action: manipulation`. IA: Timeplan\|Rapporter · Innboks\|Jobber · Kunder\|Organisasjon · Hjelp (hopp Samarbeid) · Lager. ⛔ Chrome/PhoneShell/sidebar/Ronny. ⛔ Galaxy/Grainient/`#111` på hjem-kort. ⛔ Mekaniker-hjem. ⛔ Attio-tabell/sidebar (#138). Fasit: `docs/endwise-forhandler-hjem-apple-hard-fasit.md`. **01.09.2026:** Forhandler-info er `ForhandlerInfoKort` (vanlig kort). Grainient er slettet. **01.09.2026 kveld:** kortene er ikke meny — meny er samme sidebar-overlay. Fast toppbar. Ingen bevel, ingen tab bar. |
...
`@endwise/ui/icons.ts` er **én barrel** — apper tar aldri inn `lucide-react` eller
`disarto-icons-react` direkte. Chrome/nav/hjem er Disarto Regular (vendorisert SVG,
`createLucideIcon`). Fem ikoner uten Disarto-treff står som lucide eller eier-SVG
(`hard-hat` · `handshake` · `bike` · `sailboat` · `clock-arrow-up`).
