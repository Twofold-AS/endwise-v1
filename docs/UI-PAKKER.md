...
| Forhandler pulse-hjem (`_shell/phone-home-dealer.tsx`, `_shell/pulse-kort.tsx`, `_shell/phone-home-pulse.ts`, F3-05 08.09.2026) | **Ingen ny pakke.** Fem flater + Analyser nederst. Toppkort: Planlagt/Pågår/Ferdig + Avvik/Forespørsel-ikoner øverst til høyre + `PulseDagSirkel` (ink `#141414`, canvas `#ffffff`, 08–20 Oslo fra `VERKSTED_DAG_START/SLUTT`). Ringen er egenskrevet: shadcn `Progress` er lineær, Amicro-donut er dither-graf og forbudt på dette kortet. Ikoner = lucide `TriangleAlert` / `MessageSquare` i hvit 36px-plate. Avvik → stub `/avvik` (F7-05). Forespørsel → `/hjelp?fane=forespor`. Analyser-stripe (Se tall + Amicro-rutenett) ligger **under** Innboks/Lager/På jobb/+Jobb. Ikonplater `#ffffff`. ⛔ `#0066ff` på sirkelen. ⛔ donut på hjem. ⛔ mini-stats/trend. ⛔ gamle pulse-kort. ⛔ #114/#119. Chrome og Ronny/profil urørt. |
