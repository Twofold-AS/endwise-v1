...
> (`--ew-ink-utility` / `--primary`). Lenke/pip er Synara-terracotta `#a43a0a` (mørkt `#ffb27f`).
> Warm stone page `#f5f4f2` · card `#fbfaf8` · ink-CTA `#1c1917` / `#fafaf9`.
> Geist + Geist Mono (Inter fallback). Dual theme: toggle + system, `.dark` på html.
> ⛔ Attio Action Blue `#407ff2` som primær. ⛔ Linear acid-lime. ⛔ Attio-layout / Fluid / frost (#138).
> ⛔ Ronny-kropp / logogrønn `#1ED27D`. Switch-track følger aksent.
...
> Geist · titler 16/20 Semibold · labels 13/16 · brødtekst 17/1.47 · knapper 32px / pille-CTA ·
...
| **Farger** | Attio-hex i kallstedet: ink `#1c1d1f`, Action Blue `#407ff2`, Focus Blue `#94b9ff`, Overcast `#8f99a8` |
...
| Pakke | Rolle |
|---|---|
| `@endwise/widget-tokens` | `--ew-*`-tokens (lys/mørk/aksent). ✅ **07.09.2026 Synara dual theme.** Lys `#f5f4f2` / `#141413` / `#fbfaf8` / CTA `#1c1917` / lenke `#a43a0a`. Mørk `#121110` / `#f5f4f2` / `#1a1918` / CTA `#fafaf9` / lenke `#ffb27f`. Geist. `.dark` + `data-theme`. ⛔ Action Blue. ⛔ acid-lime. Logogrønnen `#1ED27D` er merkevare i logo.svg. |
...
|---|---|---|
| **Font** | Geist (Inter fallback) | `--ew-font-sans` |
...
| **Knapper** | 32px høyde · CTA-pille 9999 · utility 10px | `h-control` · `rounded-pill` / `rounded-control` |
| **Datarad** | 40px | `h-row` |
...
| **Tekst** | ink `#141413` · secondary `#5b5955` | `text-fg` · `text-fg-muted` |
| **Lyst** | page `#f5f4f2` · sidebar/kort `#fbfaf8` · valgt `#e8e6e2` | `bg-bg` · `bg-sidebar` · `bg-sidebar-active` |
| **Mørkt** | page `#121110` · card `#1a1918` · text `#f5f4f2` | samme utilities |
...
bruker `logo.svg` uten markeds-masken. `--primary` / `--ew-ink-utility` er **ink `#1c1917`** (CTA-fyll; mørkt `#fafaf9`).
`--ew-accent` er **terracotta `#a43a0a`** (lenker/pip; mørkt `#ffb27f`).
Ikke Action Blue, ikke acid-lime, ikke grønn, ikke roadmap-rød `#EE2924`. ⛔ Ronny-kropp.

`--ew-success` (`#15B042`) er informasjon, ikke knappfarge.
`--ew-sidebar-section` er seksjonsetikett. `--ew-accent-pip` er aktiv-pip.
...
### ✍️ Typografi — GEIST (07.09.2026)

**Erstatter Inter som primær** (Inter lastes fortsatt som fallback). Historikken står i
`docs/roadmap-endringer.md`; dette er gjeldende.

| | |
|---|---|
| **Sans** | **Geist** · variabel · subsets `latin` + `latin-ext` (æøå ✓) |
| **Mono** | **Geist Mono** · tall/tabeller (`tabular-nums`) |
| **Fallback** | Inter + JetBrains Mono (neste/font, ikke primær) |
| **Lisens** | **SIL Open Font License (OFL)** |
| **Kilde** | Google Fonts via **`next/font/google`** i `apps/web/app/layout.tsx` — selvhostet ved build |
| **Variabler** | `--font-geist-sans`, `--font-geist-mono` (på `<html>`) → `--ew-font-sans/-mono` |
...
| `ProduktRamme` på markedssiden (`apps/web/app/_markeds/`, F5-35, Jonas 05.09.2026) | **Ingen ny pakke.** Fast spor: `aspect-[16/10]` / `aspect-[9/19]`, `rounded-[14px]`. Uten `kilde` = Synara 3-kolonne Endwise-mock. **07.09.2026:** primær CTA er token-ink-pille (`--ew-ink-utility`), ikke Action Blue. Merke er `bg-fg`. `TemaToggle` i nav. |
