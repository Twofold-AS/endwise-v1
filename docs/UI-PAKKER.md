...
> ### ⚠️ Mikael PR #178 — Claude på ekte ruter (25.09.2026)
> Bit 1–7 + Fix A ligger på produkt-rutene (`/home`, `/kunder`,
> `/jobber`, `/innboks`, `/bookinger/ny`, `/lager`, `/butikk`,
> `/prisliste`, `/organisasjon`). `/pr-review` og PR-only
> `*-preview` er fjernet. Eldre GO-previews fra main beholdt
> (`/pulse-preview`, `/innboks-preview`, `/visual-forms-preview`,
> `/ia-chrome-preview`). Ingen nye chrome-piller. Ingen bypass.
> ⛔ ikke merge #178.
>
...
> varighet/pris/skills live. **Ingen Claude Tjenester-chrome-pille.**
> Org-hub på `/organisasjon`: Ansatte · Timeplan ansatte (`/jobber`) ·
> Abonnement · Integrasjoner. Firmaopplysninger fra `forhandler.get`.
> Ansatte `N ansatte · M på jobb nå` fra `team.list`-status (bookinger
> i dag). Sett tid = «ingen API». Slett = `team.fjern`. Vaktliste per
> ukedag = ærlig tom. Integrasjoner uten av/på-brytere.
> Live: `/prisliste` · `/organisasjon`. Ingen ny pakke.
> ⛔ #176/#177. ⛔ Hellanor/Mailchimp live. Venter Mikael browser-review.
>
> ### ⚠️ Mikael Claude Design BIT 6 — Lager + Butikk (24.09.2026)
> Lager-hub på eksisterende `/lager`: stats På lager · Tilgjengelig ·
> Reservert · Under minimum (`inventory.summary` + `listParts` kunLav).
> Seksjoner Deler · Inn- og utlogg · Bestill · Kjøretøy til salgs.
> Deler-detalj mot `inventory.part`. Ny del = `createPart`. Bestill og
> salgskanaler (Finn.no · Butikk · Reservert) = ærlig «ingen API».
> Butikk-hub: pageTitle Butikk, `N varer · M kjøretøy`, preview 3 +
> Se alle mot `shop.catalog`. Katalog + kasse + widget urørt.
> Chrome-piller urørt. Live: `/lager` · `/butikk`.
> Ingen ny pakke. Bit 7 samme PR. ⛔ #176/#177. ⛔ Hellanor/MC-Import live.
>
> ### ⚠️ Mikael Claude Design BIT 5 — Ny jobb / jobSlots (24.09.2026)
> `jobSlots()`: hverdag 08–19, lør 10–15, søn stengt, 30-min steg,
> alle aktive mekanikere (ingen vakt-API), kval fra tjeneste-skills,
> ingen overlap mot `bookings.calendar`. Dato bare dager med slot.
> Wizard-rekkefølge additivt på `/bookinger/ny`. Kjøretøy-kaskade
> type→merke→modell→år fra `vehicles.list` (tom = fritekst). Nested
> park midt i jobb/kunde. `?kunde=` prefill. `bookings.create` urørt.
> Chrome Timeplan · Opprett jobb · Endringer urørt. Preview:
> `/visual-forms-preview` · live `/bookinger/ny`. Ingen ny pakke.
> ⛔ Bit 6–7. ⛔ #176/#177.
>
> ### ⚠️ Mikael Claude Design BIT 4 — Innboks / samtale (24.09.2026)
> Filter-chips **Alle · Kunder · Intern · Support · Løst** i lista
> (ikke chrome-piller). Løst er ærlig stub (ingen resolved-kolonne).
> Sortering-ark: Nyeste · Eldste · **Uleste først**. Pager 10 +
> sesjon-**Angre** på skjul (ikke fake delete-API). Rader: unread-border
> + `listThreads.sisteTekst`. Tråd: deltaker-chips. Inviter =
> `messages.forkThread`. Fjern / Forlat / Løst = «ingen API».
> Chrome 1+2 + `Alle meldinger · Ny melding · Sortering · Slett` urørt.
> PromptInput compose urørt. Preview: `/innboks-preview`. Live: `/innboks`.
> Ingen ny pakke. ⛔ Bit 5–7. ⛔ #176/#177.
>
...
> og booking-status urørt. Live: `/jobber`.
...
> Søket `shrink-0` (ingen grå flex-1-stripe). Pager/filter-UX vekk
> fra lista (hjelpere i `_alfa.ts` beholdt). Live: `/kunder`.
> ⛔ ikke merge #178.
>
...
> `customers.byId`. Skjemaer (`NyKunde` / `RegistrerKjoretoy` /
> `KundeEndre`) urørt. Chrome 1+2 + `FORHANDLER_NAV` urørt. Ingen ny
> pakke. ⛔ #176/#177. ⛔ Bit 3–7.
>
> ### ⚠️ Mikael Claude Design BIT 1 — Verkstedet / hjem Tall (24.09.2026)
> Hjem-Analyse-kortet heter **«Tall»**: Innboks-hode + **2×2** Visninger ·
> Bookinger · Returer · Credits. CTA **«Alle tall»** → `/statistikk`.
> Bookinger = live 30d. Visninger/Returer/Credits = ærlig «Ingen API».
> **Beholdt:** Planlagt/Pågår/Ferdig, 08–19-halvsirkel, Avvik/Forespørsel,
> Endringer, Innboks, Lager, På jobb, Jobb. **Ikke slettet:** Amicro /
> `AnalyseKort` / `/analyse`. Chrome 1+2 + `FORHANDLER_NAV` urørt. Ingen ny
> pakke. ⛔ #176/#177-omskriving (Deler/Svarhastighet/Gulv). ⛔ #114/#119.
>
...
> `08.00` / `19.00`. Analyser = to loddrette bokser (Analyse + fire KPI med
> % grønn/rød) — **ingen** dither/Amicro i del 2. Hjem `gap-2.5` så flaten
> holder seg i ett viewport. Destinasjoner: original logo-bar alltid;
...
> Avvik og Forespørsler er **to separate** Modus-sirkler (ikon i `ew-modus-plate`
> + `p-0.5` + `size-7` / `h-7`) med **tall ved siden** — ikke én avrundet boks
> med loddrett skille. Samlet **maks 50 %** av raden. Planlagt/Pågår/Ferdig
> **loddrett sentrert** mellom ukedag+dato og Avvik-raden. Dagsfremgang =
> **kun** Amicro `DitherDonutChart` (`sweep={π}`), fyll `#0066ff` for hvor
> full dagen er, fot `08.00` / `19.00`. Ingen ekstra SVG-strek/nål. Analyser:
> `Tall for {forhandler}` uten anførsel/guillemets, Innboks-radhøyde
> (`min-h-11`), Amicro **Revenue Line** uten hover/klikk (`pointer-events-none`).
> 50/50 over Jobb / På jobb. Eksisterende vendor Amicro (CLI-bin mangler).
> Ingen ny pakke. ⛔ #114/#119. ⛔ én delt Avvik|Forespørsler-boks. ⛔ zoomet
> dither-rutenett. ⛔ Boring dither-kit.
>
...
> Planlagt/Pågår/Ferdig, etiketter **08 … 19**. Avvik | Forespørsler er **én
> visningsboks** (rød/gul, ikke knapper). Hvit **Endringer**-knapp →
> `/jobber?fane=endringer` (Innstillinger-chrome + inner Avvik/Forespørsler).
> Innboks: Settings-chrome + top-bar 2 (Ny melding · Tid-popup · Gruppe
> Kunder/Internt/Support · Slett). Org-oversikt = Konto-rader. Profil:
> **Ditt abonnement** 17/700. Amicro `sweep` i `dither-donut`. Ingen ny pakke.
> ⛔ #114/#119. ⛔ `#0066ff` på dagsbuen. ⛔ Modus-plater på toppkortet.
>
...
> ### ⚠️ Mikael CODE-GO toppkort polish (08.09.2026 natt)
> Dither-sirkel **over** Endringer. Vindu **08–19** Oslo (ikke Timeplan 20).
> Bue `startAngle={π}` — klokkevis fra venstre mot høyre. Nederst på samme
> boks: **Avvik** → `/avvik` og **Forespørsler** → `/hjelp?fane=forespor`.
> Del 1 og Endringer-teller urørt. Jobb-rad: etikett venstre, ikon høyre.
> Ingen ny pakke. ⛔ #114/#119. ⛔ `#0066ff` på dagsirkelen. ⛔ mock-badge.
>
> ### ⚠️ Mikael CODE-GO Endringer + innboks + Ronny (08.09.2026 kveld)
...
> ⛔ #114/#119. ⛔ `#0066ff` på dagsirkelen. ⛔ mock på Endringer-badge.
>
> ### ⚠️ Mikael CODE-GO hjem toppkort (08.09.2026)
> Avvik + Forespørsel-ikoner sitter **på uke-kortet** (øvre høyre), ikke som
> side-chrome. Dither/statistikk-feltet **i samme kort** er byttet mot en
> ink-sirkel for verksteddagen (08–20 Oslo, Timeplan-lås). Analyser-stripen
> flyttes **nederst** under Innboks / Lager / På jobb / +Jobb. Ingen ny pakke.
> shadcn Progress er lineær; Amicro-donut er dither — ringen er egenskrevet.
> ⛔ #114/#119. ⛔ `#0066ff` på sirkelen.
>
> ### ⚠️ Mikael CODE-GO home + Hjelp (08.09.2026)
> Hjem: uke-spark («Denne uken», 7 dager), sentrerte Planlagt/Pågår/Ferdig,
> mer dither-fyll, **ingen mock-badge**. På jobb = aktiv tildeling
> (`aktivJobb`), ikke timeføring. Ny Analyser-blokk (Se tall + mock
> nettsidevisninger) over Innboks. `/statistikk` og `/hjelp` deler
> Innstillinger-chrome (midtstilt tittel + underline-faner).
> Profil: **Hjelp** (ikke Forespørsel); Vilkår **samme 17/700** som rader.
> Innboks-stripe: Ny samtale · ett sort midt · velg+slett høyre.
> Ingen ny pakke. Amicro `DitherGrowthChart` + shadcn `Badge`/`DropdownMenu`.
> ⛔ gamle pulse-kort. ⛔ #114/#119.
>
...
> Toppkort-spark er Amicro `DitherGrowthChart` linje/growth (forrige →
> denne måned) i mini avrundet boble — **ikke** `DitherDonutChart` / sirkel.
> Ikonplater på Innboks / Lager / ansatte / +Jobb er alltid hvit `#ffffff`
> (følger ikke tema). + Jobb er én linje (hvit boks + «Jobb»). Innboks kun
> meldingstall — ingen mini-stats/trend. Ikoner 22px. Ingen ny pakke.
> ⛔ Ronny/profil-popup. ⛔ #114/#119.
>
...
>
> ### ⚠️ Mikael telefon-chrome + Konto (08.09.2026)
> Top-bar 2 har mer luft (`PHONE_BAR2` py-2.5). Profil-sirkel `bg-fg text-bg`.
> Søk-felt: hvit/ink `Search`-ikon (`text-fg`). Overlay uten dest-ikonstripe
> og uten store dest-rader — kategoriserte treff fra `search.global`.
> Profil-bokstav åpner Mobbin-meny (scroll, `#0066ff` Oppgrader-CTA).
> Settings: tilbake-pil + «Innstillinger» + underline-nav; Konto-fane med
> stille avatar og rad+Endre. Ingen ny pakke. ⛔ #114/#119.
>
...
> flush venstre · **Avbryt** · nylige (vertikal) · horisontale dest-ikoner
> · store dest-rader. Felt tint `#f0f0f0` / `bg-inset`. Sheet: tettere
> håndtak, kompakt bare «Ronny» (ingen bot), full = forminsk / Ronny på
> logg-topp. Dest-nav-fiks beholdt. Ingen ny pakke. ⛔ blå CTA. ⛔ pip.
> ⛔ #114/#119.

> ### ⚠️ Mikael telefon-chrome — to toppbarer (07.09.2026)
> På **telefon/smalt viewport** forsvinner dealer-sidebaren. Chrome er to
> toppbarer + hårlinje: 1) merke + Mobbin-søk (`bg-inset` `#f0f0f0`,
> `rounded-sm` 16px, uten hvilekant) + Ronny-sirkel + profil-bokstav i
> samme 28px-sirkel. 2) Horisontale dest-piller fra `FORHANDLER_NAV` /
> `navForShell` (Verkstedet, Innboks, Timeplan, …). Aktiv = canvas-soft,
> **ingen** pip / `border-left`. Hårlinje `#e0e0e0` under bar 2.
> Hjem-innhold = fem flater (Planlagt/Pågår/Ferdig + månedsboble ·
> Innboks-rad · Lager-rad · ansatte/totalt + Jobb) i Mobbin-kort
> (`rounded.md` 24). Ingen footer. Chrome urørt.
> Desktop 389/598/452 + sidebar urørt. ⛔ overlay-toggle. ⛔ midtstilt logo.
> ⛔ tilbake i top-bar 1. ⛔ nye IA-destinasjoner. ⛔ #114/#119.
...
> ⚠️ **AKSENT-ROLLER ER MOBBIN (07.09.2026, Mikael CODE-GO / Jonas+Mikael pip-lås).** CTA-fyll er ink `#141414`
> (`--ew-ink-utility` / `--primary`). `#0066ff` er KUN Popular/savings — aldri CTA, pip eller fokus-bar.
> Canvas `#ffffff` · canvas-soft `#f3f3f3` · field/hairline-soft `#f0f0f0` · hairline `#e0e0e0`.
> Inter variabel 650/450/300. Dual theme: mørkt = ink↔canvas, ikke Synara-stein.
> Aktiv nav = canvas-soft tint, **ingen** border-left / accent-pip.
> ⛔ Synara-stein/terracotta. ⛔ Attio/Apple Action Blue som primær. ⛔ Linear acid-lime.
> ⛔ Ronny-kropp / logogrønn `#1ED27D`. Switch-track følger ink.
...
> Inter 650/450/300 · titler 16/20 · labels 13/16 · brødtekst 17/1.38 · knapper 32px / pille-CTA ·
> rader 40px (data) og 44px (stores) · badge 20px/6px · switch 24×14/10px · tekst ink/muted ·
> **DUAL THEME** (lys + mørk). `TemaToggle` + `prefers-color-scheme`.
...
| **Data** | Amicro dither charts | Søyle (stacked), areal (growth), linje (spline), donut. Analyse/Rapporter. ⛔ dither-kit |
...
## 2. Amicro dither charts — produkt-grafer (06.09.2026)

| | |
|---|---|
| **Brukes til** | Analyse / Rapporter (F5-18): bookingvolum, belegg, sidevisninger, trafikkilder |
| **Installasjon** | Vendorisert. `npx @subhanhq/amicro@latest add dither-donut` **feiler** — `@subhanhq/amicro@1.0.1` har ingen CLI-bin. shadcn-registeret har ikke dither-items. |
| **Kilde** | https://github.com/Subhan-code/Amicro--Micro-transitions- · site https://amicro.vercel.app/dither-charts |
...
| **Lisens** | MIT |
| **Hentet inn** | `DitherStackedChart` · `DitherGrowthChart` · `RevenueLineChart` · `DitherDonutChart` + `useCanvasSetup`. **09.09:** valgfri `sweep` (default 2π; hjem-dagsbue π = halvsirkel). **08.09 korreksjon:** `DitherGrowthChart` compact som mini linje/growth-boble (forrige → denne måned) på forhandler-toppkort (F3-05). ⛔ `DitherDonutChart` på hjem. **08.09:** donut som månedsboble (trukket tilbake). **07.09:** `DitherGrowthChart` compact var 7d-spark. |
| **Ikke hentet** | heatmap, gauge, scatter, funnel, device-donut, storage, uptime |
| **Farger** | Mobbin-hex i kallstedet: ink `#141414`, ink-soft `#262626`, muted `#707070`, faint `#adadad`, field `#f0f0f0`. `#0066ff` kun hjem-dagsfremgang (data viz) og uke-spark. ⛔ `#0066ff` på Analyse/Rapporter-grafer |
| **Status** | Mikael 06.09.2026. ⛔ Boring-Software dither-kit / tripwire |

`compact` er produkt-stien (kun canvas). `AnalyseKort` eier chrome, tittel og Mock-merke. Tallet står i klartekst over grafen — samme regel som før.

### 2b. Recharts — shadcn Chart-mønster (ikke motor på Analyse lenger)

Recharts-wrapperen i `packages/ui/src/components/chart.tsx` ligger igjen. Appene skal ikke hente den inn på nye flater. Analyse/Rapporter bruker §2 Amicro.

| | |
|---|---|
| **Brukes til** | Leftover. Ikke nye kallsteder — Analyse bruker §2 Amicro |
...
| **Status** | ⚠️ Ikke lenger produkt-motor på Analyse (06.09.2026 — Amicro). Filene ligger |
...
> ### Ikke bruk denne pakken. Mikael 06.09.2026: produkt-grafer er **Amicro** (§2), ikke dither-kit / tripwire.
>
...
| Pakke | Rolle |
|---|---|
| `@endwise/widget-tokens` | `--ew-*`-tokens (lys/mørk/aksent). ✅ **07.09.2026 Mobbin dual theme.** Lys canvas `#ffffff` / ink `#141414` / canvas-soft `#f3f3f3` / CTA `#141414` / aksent `#0066ff` (kun Popular). Mørk = ink↔canvas. Inter 650/450/300. Pille 9999 · kort 24 · felt 16. `.dark` + `data-theme`. ⛔ Synara-stein/pip. ⛔ Action Blue som CTA. ⛔ acid-lime. Logogrønnen `#1ED27D` er merkevare i logo.svg. |
...
|---|---|---|
| **Font** | Inter variabel 650/450/300 | `--ew-font-sans` |
| **Titler** | 16px / 20px linjehøyde / 650, null tracking | `text-title` |
| **Labels** | 13px / 16px / 450 | `text-label` |
| **Brødtekst** | 17px / 1.38 / 450, null tracking | `text-body` |
| **Knapper** | 32px høyde · stadium-pille 9999 | `h-control` · `rounded-pill` |
| **Datarad** | 40px | `h-row` |
...
| **Switch** | 24×14px track · 10px thumb · track følger ink | `<Switch>` · `--ew-switch-*` |
| **Tekst** | ink `#141414` · muted `#707070` · faint `#adadad` | `text-fg` · `text-fg-muted` · `text-fg-faint` |
| **Lyst** | canvas `#ffffff` · valgt `#f3f3f3` · felt `#f0f0f0` | `bg-bg` · `bg-sidebar-active` · `bg-inset` |
| **Mørkt** | ink `#141414` · card `#262626` · text `#ffffff` | samme utilities (polaritet) |

**Dual theme** (`TemaProvider` + FOUC-skript). Klasse `.dark` og `data-theme` på `<html>`.
Uten lagret valg: `prefers-color-scheme`. `TemaToggle` i sidebar og på landing.
...
merket med ink `#141414` / `bg-fg` — ikke `#1ED27D`. Dealer-chrome
bruker `logo.svg` uten markeds-masken. `--primary` / `--ew-ink-utility` er **ink `#141414`** (CTA-fyll; mørkt `#ffffff`).
`--ew-accent` er **`#0066ff`** — KUN Popular-badge / savings. Aldri CTA, aldri sidebar-pip, aldri fokus som blå bar.
Aktiv nav-rad er `bg-sidebar-active` (canvas-soft) uten border-left.
Ikke Synara-stein, ikke Action Blue som primær, ikke acid-lime, ikke grønn, ikke roadmap-rød `#EE2924`. ⛔ Ronny-kropp.

`--ew-success` (`#15B042`) er informasjon, ikke knappfarge.
`--ew-sidebar-section` er seksjonsetikett. Pip-token er fjernet.
...
### ✍️ Typografi — INTER (07.09.2026, Mobbin)

**Erstatter Geist.** Saans 652/456/300 → Inter variabel 650/450/300.
Fallback: `-apple-system, "Helvetica Neue", Arial, sans-serif`.

| | |
|---|---|
| **Sans** | **Inter** · variabel · subsets `latin` + `latin-ext` (æøå ✓) |
| **Mono** | JetBrains Mono · tall/tabeller (`tabular-nums`) |
| **Fallback** | `-apple-system`, Helvetica Neue, Arial |
| **Lisens** | **SIL Open Font License (OFL)** |
| **Kilde** | Google Fonts via **`next/font/google`** i `apps/web/app/layout.tsx` — selvhostet ved build |
| **Variabler** | `--font-inter` (på `<html>`) → `--ew-font-sans` |
...
|---|---|---|---|
| Tittel (H1/H2) | `text-title` | 16 / 20 | 650 |
| Label, nav, knapp | `text-label` | 13 / 16 | 450 |
| Brødtekst | `text-body` | 17 / 1.38 | 450 |
| Meta *(utledet)* | `text-[12px]` | 12 | 450 |
...
| Admin-shell + oversikt-komposisjoner (`apps/web/app/(app)/…`: `Sidebar`, `SupportCard` + `BevelButton`/`BEVEL` (TheFold-kortstil, telefon), `SectionCard`, `KpiCard`, `BookingsTable`, `RevenueTable`, `DealerList`; `_shell/nav.ts`, `_lib/use-org-role.ts` rollegate) | **Ikke** gjenbrukbare primitiver — de er app-nivå *komposisjoner* av eksisterende pakker (shadcn `Button` + `@endwise/ui`-ikoner + tokens). **06.09.2026:** desktop `TopBar` er tilbake over boks 2 (598×53 breadcrumb), `StandbyPanel` er tom boks 3 (452×53). `BookingsTable`/`RevenueTable` er vanlige `<table>`-er skrevet 03.08.2026 da dither-grafene ble fjernet — shadcn `table` kan hentes inn og erstatte dem når noen orker. De hører til `apps/web`, ikke `@endwise/ui`, så de står her kun for sporbarhet. Ingen ny pakke tatt inn |
| Sidebar-først shellet (`apps/web/app/(app)/_shell/`: `nav.ts`, `sidebar.tsx`, `sidebar-header.tsx`, `phone-shell.tsx`, `phone-home.ts`, `phone-home-dealer.tsx`, `phone-home-mekaniker.tsx`, `phone-kort.tsx`, `pulse-kort.tsx`, `phone-chrome.ts`, `phone-side-chrome.ts`, `sortering-ark.tsx`, `side-chrome-skall.tsx`, `hjem-pulse-sync.ts`, `phone-sok.ts`, `phone-sok-overlay.tsx`, `phone-profil-meny.tsx`, `phone-tema-rad.tsx`, `phone-h-scroll.tsx`, `seksjon-bar.tsx`, `seksjon-sti.ts`, `side-piller.tsx`, `oppgrader-pille.tsx`, `tilbake-pil.tsx`, `desktop-shell.ts`, `standby-panel.tsx`, `top-bar.tsx`, `people-showcase.tsx`) + destinasjonene `home/` (alias `/dashboard` → `/home`), `hjelp/` (`/support` re-eksport), `statistikk/`, `saker/`, `samarbeid/`, `analyse/`, `endwise/` (inkl. `/endwise` oversikt, `/endwise/flagg`, `/endwise/team`, `/endwise/verksted/[slug]`, F1-07/F0-04/F5-11), `innstillinger/*`, `oppstart/` (F5-26 eier-veiviser) | App-nivå **komposisjoner** (F5-13). Bygget av `@endwise/ui`. **11.09.2026 (Mikael CODE-GO):** `PulseAnalyserKort` er to loddrette bokser (Analyse + fire KPI) — Amicro/Revenue Line er ute av denne flaten. `SorteringArk` er samme fixed+scrim-plate som profilmenyen (`text-body`), brukt av Innboks/Kunder/Tjenester. Ingen ny pakke. **08.09.2026 (Mikael CODE-GO home + Hjelp):** `PulseAnalyserKort` og `SideChromeSkall` er egen komposisjon — shadcn Card/Tabs er New York-boxed og ville gitt et annet system enn Innstillinger-underline. Analyser-rutenett gjenbruker Amicro `DitherGrowthChart` + shadcn `Badge`. Ingen ny pakke. **08.09.2026 (Mikael CODE-GO + oppfølging):** overlay er nylige + `search.global`-grupper (ingen dest-ikonstripe). `PhoneProfilMeny` er egen panel (ikke shadcn DropdownMenu-rader) fordi Mobbin-menyen har blå pille-CTA + Modus-segment med sirkel på aktivt ikon — DropdownMenu dekker ikke det uten å restyles til et annet system. Modus-rad gjenbruker `TemaProvider`. Ronny fyller samme `size-7` som profil via `ronnySizeForSirkel`. Ingen ny pakke. **07.09.2026 kveld (Mikael CODE-GO):** søk/Ronny/profil 28px; `PhoneSokOverlay` (Avbryt · nylige). Ingen ny pakke. **06.09.2026 (Mikael desktop polish):** Logo 40px. Nav-ikon 20×20, label 16/20. Ronny i boks 3. Enterprise-label 18 (boks 233×52). Hjem-kort 300×140 py 16 / px 12. «Verkstedet» ut av boks 2-toppbar. Ingen ny pakke. **06.09.2026 (Mikael #142 Handlinger/toggle/mål):** «Handlinger» fjernet fra sidebar. Ingen desktop collapse/expand (telefon-overlay lukkes fra PhoneShell). Nav-boks 259×50, ikon 26×26 urørt, label 21/26. Enterprise-boks 233×52 px-32. Profil 259 pad 12. Chrome 275 flush-right og 3-kolonne beholdt. **06.09.2026 (Mikael #142):** hjem-kort 348×160 (`md:`, py 20 / px 16). People-showcase 182×40, ikon 20×19 — eksisterende `Avatar`, ingen ny pakke. Sidebar-chrome 275 flush-right i 389 (8+259+8); nav-knapp px 12, ikon venstre / tekst høyre; logo 30px via CSS på desktop-header. **06.09.2026 (Mikael desktop tre-kolonner):** 389 / 598 / 452, toppbarer 53px over boks 2+3, boks 3 tom standby. Ingen ny pakke — shadcn `sidebar` er ikke hentet (eksisterende skinne). Telefon-overlay beholder 16px-ikon og `h-control`; desktop-mål er `md:`. ⛔ Attio HTML-preview / Fluid / frost. **05.09.2026 (Mikael uttrykk + fast chrome):** Ronny-avatarer `state='idle'` + uttrykk (⛔ thinking/alert/notify). PhoneShell `fixed` z-60; telefon-sidebar-header skjult; overlay under `h-row`. **05.09.2026 (Jonas desktop Ronny):** Avatar i sidebar-header rett til venstre for toggle (`hidden md:inline-flex`). Høyre overlay-panel over main, max 400px, lett scrim, Escape/X. Ingen forstørr/handle/sheet/stripe på desktop. Telefon-sheet urørt. **05.09.2026 (Jonas/Mikael Ronny-sheet):** Telefon-toppbar = ink-logo midt, tilbake-pil med hale, Ronny-avatar + sidebar-toggle ytterst. Stripe/peek borte. Sheet 80/100 kun `md:hidden`. **05.09.2026 (dealer-chrome bugfix, delvis overstyrt):** Oppgrader-pille følger `planKey` (Enterprise = merke). Hjem-kort fylt fra eksisterende tRPC. **01.09.2026 kveld (Mikael IA+auth):** Hjelp-`TipCard` er ute. Nederst sitter Galaxy-`OppgraderPille` («Oppgrader til {neste}») eller Enterprise-merke. Forhandlernavn er ren `<h1>`, ikke kort. Telefon: sidebar-ikon ytterst til høyre, Tilbake er pil (`history.back()`). `DestinasjonSeksjonBar` under toppbaren. Samarbeid borte fra telefon-hjem. Piller er Organisasjon-stil (`bg-sidebar-active`, wrap, ingen slider). Innboks: én verktøylinje i lista, telefon skjuler lista når en tråd er åpen. Åpen tråd: dest-bar = Tilbake + slett + Inviter ansatt. Hero på Verkstedet = forhandlernavn. **01.09.2026 kveld:** overlay-sidebar, lys-only. `/bot` lever som URL, ikke i nav. |
...
| `RonnyHandtak` (`_workshop/ronny-ikoner.tsx`) | **Ingen ny pakke.** Asset ligger igjen. **11.09.2026 (Mikael CODE-GO tillegg):** ikke montert i åpen telefon-sheet — ingen strek/grabber over Ronny-ikonet. Full-åpen er `100dvh` + `top: 0` med `RONNY_SHEET_RADIUS_PX` kun i toppen. Forstørr-knapp åpner 100 %. shadcn/vaul Sheet er ikke hentet. |
...
| `OppgraderPille` (`apps/web/app/(app)/_shell/oppgrader-pille.tsx`) | **Galaxy** (§1) på **både** neste-nivå-CTA og Enterprise-merke. Oval, Galaxy klippet inne, svart `#111`. **06.09.2026 (Mikael #142):** desktop-boks 233×52, padding venstre/høyre 32 (`md:px-8`). Telefon beholder `h-9`/`px-4`. Tekst fra `nesteTier`/`oppgraderKnappetekst` + `visOppgraderCta` i `@endwise/modules/billing` — Start → «Oppgrader til Pro», Pro → «Oppgrader til Enterprise». `planKey` = Stripe-rad, ellers `tenants.plan`. **Enterprise = merke** (`data-plan-badge`) med samme Galaxy, uten lenke. Ingen priser. |
...
| Innstillinger-skallet (`apps/web/app/(app)/innstillinger/_skall.tsx`, F5-19 24–25.08.2026) | **Ingen ny pakke.** **08.09.2026 (Mikael CODE-GO):** første fane er **Konto**. Desktop-faner er underline (aktiv `border-fg`, inaktiv `text-fg-muted`) — samme mønster som telefon top-bar 2. Telefon skjuler in-page tittel/faner (`hidden md:`). Konto: stille `Avatar` + stort navn + e-post; rader Navn/E-post/Passord/Autentikator med Endre (folder eksisterende `VisningsnavnFelt` / `ByttEpostSkjema` / `ToFaktorRad`). Administrer: Logg ut overalt + rød Slett. Ingen filopplasting, ingen fargevelger. shadcn `Tabs` ikke hentet. **Team/Organisasjon er ikke en fane.** Endwise-plattform ser kun Konto. Ingen sticky Save |
...
| Kunder alfa-rail (`kunder/_alfa.ts`, `kunder/_alfa-rail.tsx`, `kunder/_alfa-liste.tsx`, F5-02 24.09.2026) | **Ingen ny pakke.** Apple Contacts-indeks i list-viewport (`sticky` + like `flex-1`-spor). Full NO `#` A–Å med dimmete tomme. Scroll-to + sticky seksjonshoder. shadcn/beUI har ingen norsk indeks-skinne. Live: `/kunder`. |
...
| Innboks Settings-chrome + top-bar 2 (`innboks/_top-bar2.tsx`, `_popup.tsx`, `_chrome.tsx`, F5-14) | **Ingen ny pakke.** Telefon: PhoneShell Settings-chrome (`bar2: 'innboks'`). Bar 2 = Ny melding · Tid · Gruppe · Slett. Tid/Gruppe = profilmeny-plate (`PHONE_PROFIL_*`), ikke chevron/DropdownMenu/Modus-piller. Slett expander med rød `Trash2` + velg-ikoner på trådene. Ny melding = Kunder-knapperad (Kunde · Intern · Support), ikke hub-piller. **24.09 (Claude BIT 4):** chips i lista, Uleste først i Sortering-ark, pager 10, sesjon-Angre, unread-border + preview. Tråd-handlinger ærlig uten resolve/leave/remove. |
| Organisasjon › Oversikt (`organisasjon/_org-rad.tsx`, `forhandleren/_kort.tsx`, `_hub.ts`, `_hub-flate.tsx`) | **Ingen ny pakke.** Samme rad + Endre som Innstillinger › Konto. <b>24.09.2026 (Claude BIT 7):</b> hub-lenker Ansatte · Timeplan ansatte · Abonnement · Integrasjoner mot eksisterende destinasjoner. Firmaopplysninger + Antall ansatte live. Vaktliste = «ingen API». |
...
| `TemaProvider` / `TemaToggle` (`apps/web/app/_lib/tema-*.tsx`, F5-10 07.09.2026) | **Ingen ny pakke.** Ikke `next-themes` (ikke i techstack). FOUC-skript + `localStorage` + `prefers-color-scheme` + klasse `.dark` / `data-theme`. Moon/Sun fra ikon-barrelen. |
