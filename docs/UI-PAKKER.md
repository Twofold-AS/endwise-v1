...
> ### ⚠️ Mikael OVERRIDE Claude-katalog (22.09.2026)
> Full Claude Mobile-logikk under eksisterende chrome: Verkstedet live
> PPF/08–19/avvik/forespør/30d, Innboks søk·filter·pager·angre, Samtale
> L/R + chips + last, Timeplan 7-dagers ukesrail + månedspopover +
> `settleChange`, Tjenester på `/prisliste` (Org-lenke, ikke ny pille),
> `jobSlots()` i Ny jobb steg 3, lager bestilt-økt + butikk kategori,
> parentF + kaskade. **Ingen ny UI-pakke.** Chrome (top-bar 1+2,
> `FORHANDLER_NAV`) urørt. ⛔ #114/#119. ⛔ Ny destinasjon.
>
...
> Lager-hub (På lager · Tilgjengelig · Reservert · Under minimum), Butikk-hub
> (3+Se alle, salgsdetalj), Ansatte På jobb/Av vakt. **Ingen ny UI-pakke.**
> Chrome (top-bar 1+2, `FORHANDLER_NAV`) urørt.
> ⛔ #114/#119. ⛔ Ny destinasjon.
>
> ### ⚠️ Mikael OVERRIDE Claude-innbygging (15.09.2026)
> Full content-flate under eksisterende chrome (pulse Tall #7, Endringer-kort,
> innboks-piller, lager-status, butikk-salg, org-ansatte/integrasjoner, 4-stegs
> Ny jobb). Pulse-base = 6 kjernekort + Tall #7. Katalog-piller (ink) under
> Endwise-chrome — ikke Claude-chrome. **Ingen ny UI-pakke.** Komposisjon av
> shadcn Dialog/Switch/Button, `CardShell`, `InnstillingSeksjon`, `SorteringArk`,
> `PHONE_DEST_FYLL`. Egen kode i `apps/web/app/(app)/_innbygging/`. Chrome
> (top-bar 1+2, `FORHANDLER_NAV`) urørt.
> ⛔ #114/#119. ⛔ Ny destinasjon.
...
| Claude-innbygging innhold (`apps/web/app/(app)/_innbygging/`, 15–22.09.2026 Mikael OVERRIDE) | **Ingen ny pakke.** 22.09: `jobSlots` + `JobSlotsVelger`, innboks-katalog (søk/pager/angre), ukesrail + månedspopover, parentF/kaskade, lager session-bestilt, butikk kategori-grupper. 20.09: Kunder-alfa er høyre rail (ikke wrap), liste `pr-[30px]`. 18.09: Kunder-liste/profil/opprett, Lager-hub, Butikk-hub + salgsdetalj, Ansatte På jobb/Av vakt — komposisjon av `PhoneSokFelt`, `InnstillingSeksjon`, `StatusMerke`, `CardShell`. Egen kode fordi Claude-katalogene (alfa-rail, pageSub, pager, hub-kutt, jobSlots-UI) ikke finnes i shadcn. Chrome urørt. 15.09: `Dialog`/`Switch`/`Button`, `innholdPilleKlasse`, Tall #7. |
