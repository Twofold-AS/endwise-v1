...
> Ny jobb). **Ingen ny UI-pakke.** Komposisjon av shadcn Dialog/Switch/Button,
> `CardShell`, `InnstillingSeksjon`, `SorteringArk`, `PHONE_DEST_FYLL`. Egen
> kode i `apps/web/app/(app)/_innbygging/` fordi ingen pakke dekker Endwise-
> spesifikke kort/piller. Chrome (top-bar 1+2, `FORHANDLER_NAV`) urørt.
...
| Claude-innbygging innhold (`apps/web/app/(app)/_innbygging/`, 15.09.2026 Mikael OVERRIDE) | **Ingen ny pakke.** Komposisjon av shadcn `Dialog`/`Switch`/`Button`, `CardShell`, `InnstillingSeksjon`, `StatusMerke` (myk tint, ingen pip). Tall-kort 2×2, Endringer-kort, innboks-piller, lager-ark, butikk-salg, integrasjon-rader. Egen kode fordi ingen pakke dekker Endwise-katalogene (status, endringstyper, Mobbin 24). Chrome urørt. |
