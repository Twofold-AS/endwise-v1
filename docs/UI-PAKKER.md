...
| Kastbar HIG-preview på hjem (`_shell/hjem-hig.css`, `hjem-hig.ts`, `hjem-hig-ramme.tsx`, F5-56 01.09.2026) | **Ingen ny pakke.** shadcn/beUI har ingen «HIG-rolle-tokens på én rute». Scoped CSS under `[data-hjem-hig]` remap'er lerret/grouped/label uten å røre `widget-tokens` (globale `#000`/`#fff` står). Ikke SF Pro, ikke Apple Blue, ikke glass. Isolert slik at sidebar og andre ruter er identiske med main. |
