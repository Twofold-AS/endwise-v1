...
| Telefon kort-hjem + mekaniker-accordion (`_shell/phone-home*.tsx`, `_shell/phone-kort.tsx`, `_shell/phone-shell.tsx`, F5-13 29.08.2026) | **Ingen ny pakke.** Destinasjonskort er samme token-komposisjon som `CardShell` (`bg-card` / `text-fg` / `border-border` / `rounded-xl`) + ikoner fra `@endwise/ui`. Ikke `bg-accent`/`text-accent-fg`: `--color-accent` er shadcn-hover (`--ew-surface-2`) mens `accent-fg` er invertert merketekst — det ga vasket hvit-på-grå i prod. shadcn `Card` er samme outline/surface og ville restyles uansett. Mekaniker «Detaljer» er eksklusiv accordion i React-state (én åpen) — shadcn `Accordion` er ikke hentet. Innstillinger-tannhjul sitter i telefon-bevelen (lucide `Settings`), ikke i rutenettet. Book for kunde på Verkstedet-jobbkortet er merkelapp, ikke gjeste-booking. |
