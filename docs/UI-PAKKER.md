...
> Hjem-innhold = pulse-kort (I dag · Innboks · Deler · Svarhastighet ·
> Timeplan-gulv · Team) i Mobbin-kort (`rounded.md` 24, tint-fill hero
> uten kant, hvilekort hairline-soft). Footer-tekst Organisasjon · Hjelp.
...
| **Lisens** | MIT |
| **Hentet inn** | `DitherStackedChart` · `DitherGrowthChart` · `RevenueLineChart` · `DitherDonutChart` + `useCanvasSetup`. **07.09:** `DitherGrowthChart` compact også som 7d-spark på forhandler I dag-kort (F3-05). |
...
| Forhandler pulse-hjem (`_shell/phone-home-dealer.tsx`, `_shell/pulse-kort.tsx`, `_shell/phone-home-pulse.ts`, F3-05 07.09.2026) | **Ingen ny pakke.** Seks operative kort + footer-tekst. Flate = eksisterende Mobbin-tokens (`PHONE_HERO_FYLL` / `PHONE_DEST_FYLL`). Spark = Amicro `DitherGrowthChart` compact (allerede i §2). Mangel på historikk: shadcn `Badge` `variant="secondary"`-pille `mock` (canvas-soft / `text-fg-muted`, ikke `#0066ff`). ⛔ «For lite data». shadcn Card er New York-boxed og ville krevd restyling. ⛔ dither-kit. ⛔ pip/border-left. ⛔ Hjelp/Organisasjon som store kort. Chrome urørt. |
