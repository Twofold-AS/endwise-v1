...
> ### ⚠️ Grainient er ute (01.09.2026)
> Grainient (`react-bits` + `ogl`) er slettet. Ikke importer `@endwise/ui/grainient.css`.
> Forhandler-info er et vanlig kort. Workshop-stripen bruker **ShaderGradient**
> (`@shadergradient/react` 2.4.20 + `@react-three/fiber` ^9 + `three` ^0.185).
> Samme Next-felle som `motion`/`recharts`: pakkene må ligge i **både**
> `@endwise/ui` og `@endwise/web`, og stå i `transpilePackages`.
> WebGL mounts etter første paint (`ShaderGradientBakgrunn`).
...
| **Bakgrunn (AI-stripe)** | ShaderGradient (`@shadergradient/react`) | Workshop-boks øverst i innholdskolonnen. Se §1. Grainient er slettet. |
...
| Sidebar-først shellet (`apps/web/app/(app)/_shell/`: `nav.ts`, `sidebar.tsx`, `sidebar-header.tsx`, `phone-shell.tsx`, `phone-home.ts`, `phone-home-dealer.tsx`, `phone-home-mekaniker.tsx`, `phone-kort.tsx`, `phone-chrome.ts`, `phone-h-scroll.tsx`, `seksjon-bar.tsx`, `seksjon-sti.ts`, `side-piller.tsx`) + destinasjonene `saker/`, `samarbeid/`, `analyse/`, `endwise/` (inkl. `/endwise` oversikt, `/endwise/flagg`, `/endwise/team`, `/endwise/verksted/[slug]`, F1-07/F0-04/F5-11), `innstillinger/*`, `oppstart/` (F5-26 eier-veiviser) | App-nivå **komposisjoner** (F5-13). Bygget av `@endwise/ui`. **01.09.2026 (Mikael desktop-chrome):** hvit sidebar, logo 32 + forhandlernavn én rad, gap +2px, ingen header-divider, ingen avatar i sidebar (kollapset = kun logg ut), profil/logg ut flate. Nav: etter Kunder kommer Tjenester (`/prisliste`) deretter Organisasjon. Samarbeid, Bot og Hjelp-dest er ute. Hjelp er `TipCard` (helpdesk-slider) nederst. Ingen breadcrumb-topbar. Workshop-bloub sitter i ShaderGradient-stripe øverst i innholdskolonnen. Grainient slettet. Lys-only. **01.09.2026 kveld:** telefon bruker **samme sidebar** som desktop (fullskjerm-overlay, lukket default). Fast toppbar med `PanelLeftOpen` ytterst til høyre. PhoneBevel borte. Kort-hjem er sideinnhold, ikke meny. `/bot` lever som URL, ikke i nav. |
...
| Workshop-stripe (`_workshop/workshop-bloub.tsx` + `shader-gradient.tsx`, 01.09.2026) | **ShaderGradient-pakke** (§1). Wrapperen `ShaderGradientBakgrunn` er tynn: Mikaels customize-URL som props, client-only etter mount. Egen kode fordi pakken er canvas, ikke en ferdig «AI-boks». Chat er eksisterende workshop (`/chat/workshop`, sidekontekst, ⛔ Quick-skriving). Samme stripe på telefon (under toppbaren) og desktop — ikke en andre FAB. |
