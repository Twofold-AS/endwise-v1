...
-- onboarding.fullfor / forhandler.update / Quick som eier.
-- id, created_at, plan og kind låst (dealer kan ikke flippe pakke).
...
-- P0 dealer-skriv som eier: lås identitet/historikk. authenticated/
-- endwise_app urørt — de bruker schema-policyene.
...
create trigger customers_owner_update_guard_trg
  ...
  for each row
  execute function customers_owner_update_guard();
...
create trigger vehicles_owner_update_guard_trg
  ...
  for each row
  execute function vehicles_owner_update_guard();
...
create trigger bookings_owner_update_guard_trg
  ...
  for each row
  execute function bookings_owner_update_guard();
...
create trigger skills_owner_update_guard_trg
  ...
  for each row
  execute function skills_owner_update_guard();
...
create trigger mechanic_skills_owner_update_guard_trg
  ...
  for each row
  execute function mechanic_skills_owner_update_guard();
...
create trigger threads_owner_update_guard_trg
  ...
  for each row
  execute function threads_owner_update_guard();
...
create trigger thread_participants_owner_update_guard_trg
  ...
  for each row
  execute function thread_participants_owner_update_guard();
...
create trigger messages_owner_update_guard_trg
  ...
  for each row
  execute function messages_owner_update_guard();
...
create trigger parts_owner_update_guard_trg
  ...
  for each row
  execute function parts_owner_update_guard();
...
create trigger stock_locations_owner_update_guard_trg
  ...
  for each row
  execute function stock_locations_owner_update_guard();
