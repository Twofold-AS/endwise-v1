...
import { createBooking, SlotConflictError, transitionBooking } from '@endwise/modules/booking';
...
describeDb('booking-motor (F3-01)', () => {
  ...
  beforeAll(async () => {
    ...
    await owner.insert(schema.mechanics).values([
      { id: mechanicId, tenantId, name: 'Kari', skills: ['mc-eu'] },
      { id: otherMechanicId, tenantId, name: 'Ola', skills: ['mc-eu'] },
    ]);
    await owner
