 ...
 * F6-19 — STATUS → HUMØR.
 *
 * Avataren på mekanikerflaten speiler FAKTISKE felt (`mechanics.active` +
...
describe('utledMekanikerStatus', () => {
  it('inaktiv mekaniker er ikke tilgjengelig, uansett jobber', () => {
    expect(utledMekanikerStatus({ aktiv: false, jobberIDag: 0, kapasitet: 1 })).toBe(
      'ikke_tilgjengelig',
    );
    expect(utledMekanikerStatus({ aktiv: false, jobberIDag: 3, kapasitet: 1 })).toBe(
      'ikke_tilgjengelig',
    );
  });
...
describe('status → blobatar-humor (bibliotekets ekte uttrykk)', () => {
  it('ledig er happy, på jobb/opptatt er thinking, inaktiv er sleepy', () => {
    ...
    expect(MEKANIKER_STATUS_HUMOR.ikke_tilgjengelig).toBe('sleepy');
  });
  ...
  it('norsk label står ved siden av uttrykket — uttrykket er ikke eneste signal', () => {
    ...
    expect(MEKANIKER_STATUS_LABEL.ikke_tilgjengelig).toBe('Ikke tilgjengelig');
  });
